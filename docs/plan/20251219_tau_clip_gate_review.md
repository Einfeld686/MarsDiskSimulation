...
> [!WARNING]
> 非推奨/legacy: 現行の外部供給デフォルトは `docs/plan/20251220_optical_depth_external_supply_impl_plan.md` の optical_depth + tau_stop 方式で、τクリップ/供給ゲート前提の運用はベースラインには適用しない。必要なら比較試験に限定する。
...
## すぐやるタスク（提案）
- [x] 深層→表層律速のデモ用プリセットを追加（薄い初期 Σ + ゆるい τクリップ + deep_mixing）。
- [x] run 設定サンプルに「headroom を意図的に確保する方法」を明記。
