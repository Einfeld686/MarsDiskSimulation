...
- 実効 κ: `1.9e-5 ~ 1.2e-3 m² kg⁻¹`（`out/<run_id>/series/run.parquet` の `kappa_eff` カラム）
...
**根拠**:
- `out/<run_id>/run_config.json` に supply ブロックが記録されていない
...
1. **κ の取得と計算**
   - 初期 κ（`kappa_eff` カラム）を `out/<run_id>/series/run.parquet` から取得
...
2. **検証ポイント**
   ...
   - `out/<run_id>/run_config.json` に supply ブロックが出力されるか
...
- [x] supply ブロックが `out/<run_id>/run_config.json` に出力され、`process_overview` で有効/無効が確認できる
