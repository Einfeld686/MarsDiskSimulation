...
確認ポイント
------------
- [x] 0D と 1D の両方で実行でき、欠損列があってもクラッシュしない。
- [x] `t_sub` が 0/負値で無効化されるケースは NaN になり、プロットから除外される。
- [x] streaming 出力（chunked）でも同じ図が作成できる。
- [x] .cmd 実行時に CPU/メモリが自動検出され、計算されたメモリ上限がログに表示される。

完了条件
--------
- [x] 1 run で `tau` vs `t_sub`/`t_coll`/`t_blow` の図を生成できる。
- [x] 1D ケースでセル集約オプションが動作し、`cell_active` をフィルタできる。
- [x] 出力図に軸ラベルと単位が明記される。
- [x] Windows .cmd から可視化が完走し、動的メモリ設定が反映される。
