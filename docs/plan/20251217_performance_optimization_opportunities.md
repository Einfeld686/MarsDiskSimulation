# 数値計算高速化可能性レポート

**作成日**: 2025-12-17  
**ステータス**: 調査完了・提案  
**対象**: `marsdisk/` 数値計算モジュール全般  
**関連**: [20251212_simulation_performance_optimization.md](./20251212_simulation_performance_optimization.md)

---

## 概要

`marsdisk/` の数値計算コードを網羅的に調査し、高速化の余地を分析した。本文書は施策の優先度と技術的詳細をまとめる。

---

## 現状の最適化実装

### Numba JIT 適用状況

| ファイル | 関数名 | 説明 | 並列化 |
|----------|--------|------|--------|
| `_numba_kernels.py` | `compute_weights_table_numba` | power-law fragment weight の事前計算 | ❌ |
| `_numba_kernels.py` | `fill_fragment_tensor_numba` | fragment tensor `Y[k,i,j]` の構築 | ✅ (`prange`) |

### キャッシュ機構

- **Fragment tensor cache**: `_FRAG_CACHE` (最大8エントリ、スカラー `v_rel` の場合のみ)

---

## 高速化可能性一覧

### 🔴 高優先度（効果大）

#### 1. `np.einsum` の Numba 化

**場所**: [smol.py:255](file:///Users/daichi/marsshearingsheet/marsdisk/physics/smol.py#L255)

```python
gain = 0.5 * np.einsum("ij,kij->k", C, Y)
```

| 項目 | 内容 |
|------|------|
| **問題** | O(n³) 演算を純粋 NumPy で毎ステップ実行 |
| **推定効果** | 2〜5倍（n_bins=40 の場合） |
| **実装難易度** | 中 |
| **提案** | Numba JIT カーネルに置換し、`prange` で並列化 |

---

#### 2. 衝突カーネル計算の最適化

**場所**: [collide.py:compute_collision_kernel_C1](file:///Users/daichi/marsshearingsheet/marsdisk/physics/collide.py#L18-78)

```python
kernel = N_outer / (1.0 + delta) * np.pi * (s_sum ** 2) * v_mat / (np.sqrt(2.0 * np.pi) * H_ij)
```

| 項目 | 内容 |
|------|------|
| **問題** | `np.outer`, `np.add.outer` が毎ステップ呼ばれる |
| **推定効果** | 1.5〜2倍 |
| **提案A** | Numba JIT 化 |
| **提案B** | 入力が変わらない場合はキャッシュ |

---

#### 3. Q_D* 計算のテーブル化

**場所**: [qstar.py:compute_q_d_star_array](file:///Users/daichi/marsshearingsheet/marsdisk/physics/qstar.py#L165-194)

| 項目 | 内容 |
|------|------|
| **問題** | 速度補間を含む複雑な配列演算 |
| **推定効果** | 1.5〜3倍 |
| **提案** | Numba JIT 化 or ルックアップテーブル事前計算 |

---

#### 4. 昇華計算の最適化

**場所**: [sublimation.py](file:///Users/daichi/marsshearingsheet/marsdisk/physics/sublimation.py)

特に `p_sat` 関数群（Clausius-Clapeyron 評価）がホットスポット。

| 項目 | 内容 |
|------|------|
| **問題** | `p_sat_clausius`, `choose_psat_backend` が毎ステップ評価 |
| **推定効果** | 1.5〜2倍 |
| **提案** | テーブル補間の事前構築と Numba 化 |

---

### 🟡 中優先度（効果中）

#### 5. IMEX-BDF(1) ソルバーの反復削減

**場所**: [smol.py:step_imex_bdf1_C3](file:///Users/daichi/marsshearingsheet/marsdisk/physics/smol.py#L146-276)

```python
while True:
    N_new = (N_arr + dt_eff * (gain + source_arr - S_arr * N_arr)) / (1.0 + dt_eff * loss)
    if np.any(N_new < 0.0):
        dt_eff *= 0.5
        continue
    ...
```

| 項目 | 内容 |
|------|------|
| **問題** | 初期 `dt` 推定が保守的すぎると反復回数が増加 |
| **提案** | 適応的初期 `dt` 推定の改善 |

---

#### 6. `_blowout_sink_vector` の最適化

**場所**: [collisions_smol.py:366-379](file:///Users/daichi/marsshearingsheet/marsdisk/physics/collisions_smol.py#L366-379)

```python
mask = sizes <= a_blow
sink = np.zeros_like(sizes, dtype=float)
sink[mask] = Omega
```

| 項目 | 内容 |
|------|------|
| **現状** | ブールマスク + ゼロ配列 |
| **提案** | in-place 演算または `np.where` に変更 |

---

#### 7. PSD 状態変換の高速化

**場所**: [smol.py:psd_state_to_number_density / number_density_to_psd_state](file:///Users/daichi/marsshearingsheet/marsdisk/physics/smol.py#L25-143)

| 項目 | 内容 |
|------|------|
| **問題** | 辞書アクセス `psd_state.get()` が毎回呼ばれる |
| **提案** | dataclass への変更で属性アクセス高速化 |

---

### 🟢 低優先度（効果小〜微）

#### 8. ロギングオーバーヘッドの削減

**場所**: `dynamics.py` の `v_ij`, `v_rel_pericenter` 等

```python
logger.info("v_ij: e=%f i=%f v_k=%f -> v_rel=%f", e, i, v_k, v_rel)
```

| 項目 | 内容 |
|------|------|
| **問題** | `logger.info` が毎回呼ばれる（引数評価のオーバーヘッド） |
| **提案** | `if logger.isEnabledFor(logging.DEBUG):` ガードを追加 |

---

#### 9. 入力検証の重複削減

多くの関数で `np.asarray` と形状チェックが重複。

| 項目 | 内容 |
|------|------|
| **提案** | 上流で一度検証し、内部関数では省略可能フラグを追加 |

---

#### 10. 単精度 (float32) 対応

現状はすべて `float64`。

| 項目 | 内容 |
|------|------|
| **注意** | 数値精度の劣化リスクあり、慎重な検証が必要 |
| **提案** | 精度要件が緩いフェーズでのみ検討 |

---

## アルゴリズムレベルの最適化候補

| 施策 | 説明 | 効果 | リスク |
|------|------|------|--------|
| **アダプティブビン数** | 初期は粗いグリッド、必要領域のみ細分化 | 中〜大 | 実装複雑 |
| **スパースカーネル** | `C[i,j] ≈ 0` が多い場合にスパース行列使用 | 中 | 条件次第 |
| **時間ステップ最適化** | 安全係数の動的調整 | 小〜中 | 質量誤差リスク |

---

## 並列化オプション

| 手法 | 現状 | ポテンシャル |
|------|------|--------------|
| **Numba prange** | fragment tensor のみ | 衝突カーネル、einsum 等に拡張可能 |
| **マルチプロセス** | スイープで実装済み (`--jobs N`) | 効果的に活用中 |
| **GPU (CuPy/JAX)** | 未実装 | n_bins >> 100 で有効だが現状 40 では恩恵小 |

---

## 推奨アクションプラン

1. **まず測定**: `cProfile` または `py-spy` で実際のボトルネックを特定
2. **einsum 最適化**: 最も効果が見込める
3. **衝突カーネルのキャッシュ/JIT 化**
4. **Q_D* テーブル事前計算**

---

## プロファイリングコマンド

```bash
# cProfile で測定
python -m cProfile -s tottime -m marsdisk.run --config configs/base.yml 2>&1 | head -50

# py-spy でフレームグラフ
py-spy record -o flame.svg -- python -m marsdisk.run --config configs/base.yml
```

---

## 関連ドキュメント

- [20251212_simulation_performance_optimization.md](./20251212_simulation_performance_optimization.md) — 実行時間短縮計画（施策詳細）
- [marsdisk/physics/_numba_kernels.py](file:///Users/daichi/marsshearingsheet/marsdisk/physics/_numba_kernels.py) — 既存 Numba カーネル
- [marsdisk/physics/smol.py](file:///Users/daichi/marsshearingsheet/marsdisk/physics/smol.py) — IMEX ソルバー
- [marsdisk/physics/collide.py](file:///Users/daichi/marsshearingsheet/marsdisk/physics/collide.py) — 衝突カーネル
