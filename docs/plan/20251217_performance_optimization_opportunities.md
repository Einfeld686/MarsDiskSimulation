# 数値計算高速化可能性レポート

**作成日**: 2025-12-17  
**ステータス**: 調査完了・提案  
**対象**: `marsdisk/` 数値計算モジュール全般  
**関連**: [20251212_simulation_performance_optimization.md](docs/plan/20251212_simulation_performance_optimization.md)

---

## 概要

`marsdisk/` の数値計算コードを網羅的に調査し、高速化の余地を分析した。本文書は施策の優先度と技術的詳細をまとめる。

---

## プロファイル結果（実測）

**測定条件**: `configs/base.yml`, `t_end_years=0.01`, `n_bins=40`  
**総実行時間**: 98.4秒 (約158,000ステップ)

### tottime（自己時間）上位

| 順位 | 関数 | tottime (s) | 割合 | 備考 |
|------|------|-------------|------|------|
| 1 | `run.py:run_zero_d` | 16.2 | 16.5% | メインループのオーバーヘッド |
| 2 | `numpy.ufunc.reduce` | 4.1 | 4.2% | 配列演算の集約 |
| 3 | `tables.py:interp` | 3.5 | 3.6% | Q_pr テーブル補間 |
| 4 | **`c_einsum`** | **3.4** | **3.5%** | `np.einsum` 本体 |
| 5 | `radiation.py:qpr_lookup` | 3.4 | 3.5% | Planck Q_pr ルックアップ |
| 6 | `fromnumeric.py:_wrapreduction` | 3.3 | 3.4% | NumPy reduce ラッパー |
| 7 | `collisions_smol.py:step_collisions_smol_0d` | 2.4 | 2.4% | 衝突ステップ本体 |
| 8 | `collide.py:compute_collision_kernel_C1` | 2.0 | 2.0% | 衝突カーネル構築 |
| 9 | `smol.py:step_imex_bdf1_C3` | 1.6 | 1.6% | IMEX ソルバー |
| 10 | `psd.py:compute_kappa` | 1.2 | 1.2% | κ 計算 |

### cumtime（累積時間）上位

| 順位 | 関数 | cumtime (s) | 割合 | 備考 |
|------|------|-------------|------|------|
| 1 | `run_zero_d` | 97.0 | 98.6% | シミュレーション全体 |
| 2 | `step_collisions` | 24.4 | 24.8% | **最大ホットスポット** |
| 3 | `_resolve_blowout` | 23.4 | 23.8% | Q_pr 呼び出し含む |
| 4 | `qpr_lookup` | 21.3 | 21.7% | テーブル補間含む |
| 5 | `tables.py:interp` | 9.4 | 9.6% | 2D 補間 |
| 6 | `step_imex_bdf1_C3` | 7.7 | 7.8% | IMEX ソルバー |
| 7 | `logging.info` | 6.4 | 6.5% | ロギング |
| 8 | `compute_collision_kernel_C1` | 4.3 | 4.4% | 衝突カーネル |
| 9 | `np.einsum` | 3.4 | 3.5% | gain 計算 |

### 分析

1. **最大ボトルネックは `qpr_lookup` (21.3s, 21.7%)**
   - Q_pr テーブルの2D補間が毎ステップ1,262,314回呼ばれる
   - 補間自体 (`tables.py:interp`) に 9.4s
   - **推奨**: Q_pr 値をステップごとにキャッシュ（温度・サイズが変わらなければ再計算不要）

2. **`step_collisions_smol_0d` (24.4s累積) が第2ホットスポット**
   - 衝突カーネル構築 + IMEX ソルバー + einsum を含む

3. **`np.einsum` は 3.4s (3.5%) で当初推定より影響小**
   - 全体の 3.5% であり、JIT 化の効果は限定的
   - 2〜5倍の推定は過大評価

4. **ロギングオーバーヘッド 6.4s (6.5%)**
   - `logger.info` が 315,582 回呼ばれる
   - **推奨**: INFO → DEBUG に変更、またはガード追加

---


### Numba JIT 適用状況

| ファイル | 関数名 | 説明 | 並列化 |
|----------|--------|------|--------|
| `_numba_kernels.py` | `compute_weights_table_numba` | power-law fragment weight の事前計算 | ❌ |
| `_numba_kernels.py` | `fill_fragment_tensor_numba` | fragment tensor `Y[k,i,j]` の構築 | ✅ (`prange`) |

### キャッシュ機構

- **Fragment tensor cache**: `_FRAG_CACHE` (最大8エントリ、スカラー `v_rel` の場合のみ)

---

## 高速化可能性一覧

### 🔴 高優先度（効果大）

#### 1. `np.einsum` の Numba 化

**場所**: [smol.py:255](marsdisk/physics/smol.py#L255)

```python
gain = 0.5 * np.einsum("ij,kij->k", C, Y)
```

| 項目 | 内容 |
|------|------|
| **問題** | O(n³) 演算を純粋 NumPy で毎ステップ実行 |
| **推定効果** | 2〜5倍（未測定、類似処理の JIT 化実績に基づく推定） |
| **実装難易度** | 中 |
| **提案** | Numba JIT カーネルに置換し、`prange` で並列化 |

---

#### 2. 衝突カーネル計算の最適化

**場所**: [collide.py:compute_collision_kernel_C1](marsdisk/physics/collide.py#L18-78)

```python
kernel = N_outer / (1.0 + delta) * np.pi * (s_sum ** 2) * v_mat / (np.sqrt(2.0 * np.pi) * H_ij)
```

| 項目 | 内容 |
|------|------|
| **問題** | `np.outer`, `np.add.outer` が毎ステップ呼ばれる |
| **推定効果** | 1.5〜2倍（未測定、要プロファイリング） |
| **提案** | Numba JIT 化 |

> [!WARNING]
> `N` と `H` はステップごとに更新されるため（[collisions_smol.py:640-735](marsdisk/physics/collisions_smol.py#L640-735)）、カーネル全体のキャッシュは実質再利用不可。キャッシュ対象はサイズ配列 `s` や固定 `v_rel` に限定する必要がある。

---

#### 3. Q_D* 計算のテーブル化

**場所**: [qstar.py:compute_q_d_star_array](marsdisk/physics/qstar.py#L165-194)

| 項目 | 内容 |
|------|------|
| **問題** | 速度補間を含む複雑な配列演算 |
| **推定効果** | 1.5〜3倍 |
| **提案** | Numba JIT 化 or ルックアップテーブル事前計算 |

---

#### 4. 昇華計算の最適化

**場所**: [sublimation.py](marsdisk/physics/sublimation.py)

特に `p_sat` 関数群（Clausius-Clapeyron 評価）がホットスポット。

| 項目 | 内容 |
|------|------|
| **問題** | `p_sat_clausius`, `choose_psat_backend` が毎ステップ評価 |
| **推定効果** | 1.5〜2倍 |
| **提案** | テーブル補間の事前構築と Numba 化 |

---

### 🟡 中優先度（効果中）

#### 5. IMEX-BDF(1) ソルバーの反復削減

**場所**: [smol.py:step_imex_bdf1_C3](marsdisk/physics/smol.py#L146-276)

```python
while True:
    N_new = (N_arr + dt_eff * (gain + source_arr - S_arr * N_arr)) / (1.0 + dt_eff * loss)
    if np.any(N_new < 0.0):
        dt_eff *= 0.5
        continue
    ...
```

| 項目 | 内容 |
|------|------|
| **問題** | 初期 `dt` が過大だと負値や質量誤差で弾かれ、反復回数が増加（[smol.py:244-275](marsdisk/physics/smol.py#L244-275)） |
| **提案** | 初期 `dt` 推定を `t_coll` ベースでより正確に設定 |

---

#### 6. `_blowout_sink_vector` の最適化

**場所**: [collisions_smol.py:366-379](marsdisk/physics/collisions_smol.py#L366-379)

```python
mask = sizes <= a_blow
sink = np.zeros_like(sizes, dtype=float)
sink[mask] = Omega
```

| 項目 | 内容 |
|------|------|
| **現状** | ブールマスク + ゼロ配列 |
| **提案** | in-place 演算または `np.where` に変更 |

---

#### 7. PSD 状態変換の高速化

**場所**: [smol.py:psd_state_to_number_density / number_density_to_psd_state](marsdisk/physics/smol.py#L25-143)

| 項目 | 内容 |
|------|------|
| **問題** | 辞書アクセス `psd_state.get()` が毎回呼ばれる |
| **提案** | dataclass への変更で属性アクセス高速化 |

---

### 🟢 低優先度（効果小〜微）

#### 8. ロギングオーバーヘッドの削減

**場所**: `dynamics.py` の `v_ij`, `v_rel_pericenter` 等

```python
logger.info("v_ij: e=%f i=%f v_k=%f -> v_rel=%f", e, i, v_k, v_rel)
```

| 項目 | 内容 |
|------|------|
| **問題** | `logger.info` が毎回呼ばれる（引数評価のオーバーヘッド） |
| **提案** | `if logger.isEnabledFor(logging.DEBUG):` ガードを追加 |

---

#### 9. 入力検証の重複削減

多くの関数で `np.asarray` と形状チェックが重複。

| 項目 | 内容 |
|------|------|
| **提案** | 上流で一度検証し、内部関数では省略可能フラグを追加 |

---

#### 10. 単精度 (float32) 対応

現状はすべて `float64`。

| 項目 | 内容 |
|------|------|
| **注意** | 数値精度の劣化リスクあり、慎重な検証が必要 |
| **提案** | 精度要件が緩いフェーズでのみ検討 |

---

## アルゴリズムレベルの最適化候補

| 施策 | 説明 | 効果 | リスク |
|------|------|------|--------|
| **アダプティブビン数** | 初期は粗いグリッド、必要領域のみ細分化 | 中〜大 | 実装複雑 |
| **スパースカーネル** | `C[i,j] ≈ 0` が多い場合にスパース行列使用 | 中 | 条件次第 |
| **時間ステップ最適化** | 安全係数の動的調整 | 小〜中 | 質量誤差リスク |

---

## 並列化オプション

| 手法 | 現状 | ポテンシャル |
|------|------|--------------|
| **Numba prange** | fragment tensor のみ | 衝突カーネル、einsum 等に拡張可能 |
| **マルチプロセス** | スイープで実装済み (`--jobs N`) | 効果的に活用中 |
| **GPU (CuPy/JAX)** | 未実装 | n_bins >> 100 で有効だが現状 40 では恩恵小 |

---

## 推奨アクションプラン

1. **まず測定**: `cProfile` または `py-spy` で実際のボトルネックを特定
2. **einsum 最適化**: 最も効果が見込める
3. **衝突カーネルのキャッシュ/JIT 化**
4. **Q_D* テーブル事前計算**

---

## プロファイリングコマンド

```bash
# cProfile で測定
python -m cProfile -s tottime -m marsdisk.run --config configs/base.yml 2>&1 | head -50

# py-spy でフレームグラフ
py-spy record -o flame.svg -- python -m marsdisk.run --config configs/base.yml
```

---

## 関連ドキュメント

- [20251212_simulation_performance_optimization.md](docs/plan/20251212_simulation_performance_optimization.md) — 実行時間短縮計画（施策詳細）
- [marsdisk/physics/_numba_kernels.py](marsdisk/physics/_numba_kernels.py) — 既存 Numba カーネル
- [marsdisk/physics/smol.py](marsdisk/physics/smol.py) — IMEX ソルバー
- [marsdisk/physics/collide.py](marsdisk/physics/collide.py) — 衝突カーネル
