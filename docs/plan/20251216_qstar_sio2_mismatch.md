...
## 今回実施した修正（2025-12-13）
- `coeff_units` を qstar に追加し、既定を BA99 cgs（s[cm], ρ[g/cm^3] → erg/g → J/kg）に明示。`run_config.qstar` に採用単位系・係数表・速度クランプ件数を記録し、v<3/ v>5 km/s のクランプは初回のみ warning を出す。
- `configs/*` の qstar ブロックへ `coeff_units: ba99_cgs` を明記し、将来の係数差し替え時に単位混線しないようにした。
- 衝突カスケードを回す相分岐（固体枝のみ）は従来通りで、液体枝に衝突を追加していない。
