# 目的
- `scripts/run_sublim_windows_cooling.cmd` 実行時にメモリが 120 GB 超まで膨張した問題を整理し、再発防止の運用パラメータを決める。

# 背景・現状
- 現行テンプレート `out/run_template_sublim_smol_phase_MAX50M/config_base_sublimation.yml` は `numerics.dt_init=2.0 s`、`t_end_years=0.5`、`n_bins=40` で走っている。
...
- デフォルトの PSD 解像度は `n_bins=40`（下げる場合の下限目安は 30）。`psd_hist.parquet` は (time, bin_index, s_bin_center, N_bin, Sigma_surf) を全ステップ×全ビンで保持する。
...
- 実行前に `python -m tools.utilities.memory_probe --config out/run_template_sublim_smol_phase_MAX50M/config_base_sublimation.yml --override numerics.dt_init=auto` で行数と概算メモリを確認する。
- それでも厳しければ `sizes.n_bins` を 30 へ下げる（行数を約 25% 削減）。`psd_hist` の保持列は変えない。
- 再実行後は `series/run.parquet` の `dt_over_t_blow` を確認し、中央値≲0.1・最大≲0.2 を目安にする。`mass_budget.csv` の誤差 0.5% 以内も併せてチェック。
