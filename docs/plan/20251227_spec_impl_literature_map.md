...
| --- | --- | --- | --- | --- |
| 放射圧・ブローアウト | `E.005, E.008, E.012-E.014` | `marsdisk/physics/radiation.py`, `marsdisk/io/tables.py`, `marsdisk/run_zero_d.py` | 不一致 | E.014 の blowout 半径で Q_pr の反復解を使う点が仕様と異なる |
...
| ID | 内容 | 仕様側 | 実装側 | 影響 |
| --- | --- | --- | --- | --- |
| E14-Iter | blowout 半径の Q_pr 取扱い | `E.014` は反復なしの閉形式 | `marsdisk/physics/radiation.py` は Q_pr(s) の固定点反復を使用 | s_blow がサイズ依存 Q_pr に追従し、仕様より大きく/小さくなる可能性 |
...
### 9.6 監査チェックリスト（未監査→一致/不一致の更新）

**監査完了（不一致）**
- [x] 放射圧・ブローアウト（`E.005, E.008, E.012–E.014`）: 不一致（E14-Iter）
...
**未監査／調査中**
- [ ] 文献レジストリの DOI 欠落補完: `FegleySchaefer2012_arXiv`, `LissauerStewart1993_PP3`, `Thebault2003_AA410_685`
...
### 12.2 現時点の実装タスク（候補）
- [ ] (E14-Iter) blowout 半径の Q_pr 取り扱いを統一する（反復解を仕様に明記 or 実装を閉形式へ戻す）
