...
### この文書で扱う物理
- **放射圧による質量損失（ブローアウト）**: 小粒径ダストは放射圧で系外へ吹き飛ばされる（式 [E.013](file://analysis/equations.md#E013), [E.014](file://analysis/equations.md#E014)）
- **外部供給（supply）**: 衝突破砕や外部起源で新たに粒子が注入されるモデル（式 [E.027](file://analysis/equations.md#E027)）
- **光学的遮蔽（shielding）**: 表層が $\tau=1$ を超えると供給がクリップされる（式 [E.015–E.017](file://analysis/equations.md#E015)）

詳細な数式定義は [`analysis/equations.md`](file://analysis/equations.md) を参照してください。
...
| 用語 | 記号 | 意味 | 参照 |
|------|------|------|------|
| **光学深度** | $\tau$ | 表層の光学的厚さ。$\tau=1$ が遮蔽閾値 | [glossary](file://analysis/glossary.md) |
...
| モジュール | 役割 |
|------------|------|
| [marsdisk/physics/supply.py](file://marsdisk/physics/supply.py) | 外部供給率の計算（`get_prod_area_rate`, `_rate_basic`） |
| [marsdisk/physics/shielding.py](file://marsdisk/physics/shielding.py) | 遮蔽と $\Sigma_{\tau=1}$ クリップ |
| [marsdisk/schema.py](file://marsdisk/schema.py) | YAML 設定のバリデーション（`Supply`, `Shielding` クラス） |
| [marsdisk/run.py](file://marsdisk/run.py) | メイン実行ループ `run_zero_d` |
...
- **評価フック**: `EVAL`=1 で `scripts/research/evaluate_tau_supply.py` を各ケース後に実行し、`out/<run_id>/checks/tau_supply_eval.json` を残す。
...
- 温度テーブルや（必要な場合のみ）遮蔽テーブルのパスを変えた場合、`out/<run_id>/run_config.json` に記録された path を確認して再現性を確保する。  
- 生成物は `out/temp_supply_sweep/<ts>__<sha>__seed<batch>/T{T}_eps{EPS}_tau{TAU}/` 配下に `out/<run_id>/series/run.parquet`, `out/<run_id>/summary.json`, `checks/`（mass_budget, オプションで tau_supply_eval）, `plots/*.png` が出る。ディスク容量を事前確認すること。
...
| ファイル | 内容 | 確認ポイント |
|----------|------|--------------|
| `out/<run_id>/series/run.parquet` | 全ステップの時系列データ | `supply_rate_applied`, `headroom`, `tau_los_mars` |
| `out/<run_id>/summary.json` | 実行結果の集約 | `M_loss`, `supply_clip_time_fraction`, `case_status` |
| `out/<run_id>/run_config.json` | 実行時のパラメータ記録 | `physics_controls`, `supply` セクション |
| `out/<run_id>/checks/mass_budget.csv` | 質量収支検査ログ | `error_percent < 0.5%` |
| `out/<run_id>/checks/tau_supply_eval.json` | τ 安定性評価（`EVAL=1` 時） | 後述のパス/フェイル判定 |
...
詳細なカラム定義は [`analysis/AI_USAGE.md`](file://analysis/AI_USAGE.md) の「出力ファイルの中身」を参照してください。
...
| ドキュメント | 内容 |
|--------------|------|
| [analysis/equations.md](file://analysis/equations.md) | 全数式の定義（β, s_blow, 供給率, 遮蔽等） |
| [analysis/AI_USAGE.md](file://analysis/AI_USAGE.md) | 実行手順と出力ファイルの解釈ガイド |
| [analysis/glossary.md](file://analysis/glossary.md) | 用語集と変数命名規約 |
| [analysis/run-recipes.md](file://analysis/run-recipes.md) | 標準的な実行レシピ集 |
| [AGENTS.md](file://AGENTS.md) | プロジェクト全体の仕様と完成条件 |
