...
### この文書で扱う物理
- **放射圧による質量損失（ブローアウト）**: 小粒径ダストは放射圧で系外へ吹き飛ばされる（式 [E.013](analysis/equations.md#E013), [E.014](analysis/equations.md#E014)）
- **外部供給（supply）**: 衝突破砕や外部起源で新たに粒子が注入されるモデル（式 [E.027](analysis/equations.md#E027)）
- **光学的遮蔽（shielding）**: 表層が $\tau=1$ を超えると供給がクリップされる（式 [E.015–E.017](analysis/equations.md#E015)）

詳細な数式定義は [`analysis/equations.md`](analysis/equations.md) を参照してください。
...
| 用語 | 記号 | 意味 | 参照 |
|------|------|------|------|
| **光学深度** | $\tau$ | 表層の光学的厚さ。$\tau=1$ が遮蔽閾値 | [glossary](analysis/glossary.md) |
...
- **出力ルート**: `OUT_ROOT` 未指定なら `out/`、外付け SSD があれば `<external_out_root>` を既定にする。書き込み権限を確認。
...
- 生成物は `out/<run_id>/series/run.parquet`, `out/<run_id>/summary.json`, `out/<run_id>/checks/`（mass_budget, オプションで tau_supply_eval）, `out/<run_id>/plots/*.png` が出る。ディスク容量を事前確認すること。
...
| `out/<run_id>/plots/overview.png` | 質量損失と時間スケールの概観 | クリップ発生頻度 |
| `out/<run_id>/plots/supply_surface.png` | 供給率・表層密度・ヘッドルームの詳細 | `supply_clip_factor` が 1 未満の区間 |
...
詳細なカラム定義は [`analysis/AI_USAGE.md`](analysis/AI_USAGE.md) の「出力ファイルの中身」を参照してください。
...
| ドキュメント | 内容 |
|--------------|------|
| [analysis/equations.md](analysis/equations.md) | 全数式の定義（β, s_blow, 供給率, 遮蔽等） |
| [analysis/AI_USAGE.md](analysis/AI_USAGE.md) | 実行手順と出力ファイルの解釈ガイド |
| [analysis/glossary.md](analysis/glossary.md) | 用語集と変数命名規約 |
| [analysis/run-recipes.md](analysis/run-recipes.md) | 標準的な実行レシピ集 |
| [AGENTS.md](AGENTS.md) | プロジェクト全体の仕様と完成条件 |
