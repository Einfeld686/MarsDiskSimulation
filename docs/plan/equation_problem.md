...
## 2 表面エネルギー制約による最小粒径の式が [@KrijtKama2014_AA566_L2] と形が合わない（E.053）
...
しかし、[@KrijtKama2014_AA566_L2] の本文では、まず一般式（質量・表面積）を示し（Eqs.2–3）、そこから **(\alpha=3.5)** の特別な場合として
...
ただし [@MustillWyatt2009_MNRAS399_1403] では、ランダム化した小さな離心率の集団で平均相対速度を (v_{\mathrm{rel}}=c,e,v_{\mathrm{kep}}) と書き、**レイリー分布なら (c=\sqrt{5/4})** と明記しています。([w.astro.berkeley.edu][1])
ここまで見ると、少なくとも **(i=0) の極限では実装の (e) 項（(\sqrt{1.25},e)）は文献の形と一致**します。
一方で、[@MustillWyatt2009_MNRAS399_1403] 自身が「(c) は定義や分布に依存する」「傾斜がある場合は傾斜も効く」と注意しており、([w.astro.berkeley.edu][1])
...
ここに [@TakeuchiLin2003_ApJ593_524] の gas-rich 仮定の式を試すスイッチがあり、プロジェクトでは既定で無効、と書かれています。
...
2. **(E.053) 表面エネルギー制約の最小粒径**
   - **方針**: [@KrijtKama2014_AA566_L2] の **原典式（Eqs.2–4）を再確認**し、採用形（一般式か α=3.5 特例か）を明示する。
...
- [x] **E.053**: [@KrijtKama2014_AA566_L2] 原典式（Eqs.2–4）を再確認し、採用形と差分注記を確定する。  
