...
- 出力: `analysis/glossary_suggestions.json` を新設し、統一候補と信頼度を警告として記録。自動置換しない。
...
- 出力: `analysis/coverage_priorities.json` に順位付きリストを生成。autostub は従来どおり手動。
...
4. 実行ログ外れ値: analysis/run-recipes に追記し、出力は Git ignore。CI 非ブロックを徹底。
