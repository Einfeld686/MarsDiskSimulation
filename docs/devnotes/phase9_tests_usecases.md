...
- 衝突カスケード×昇華×ブローアウトの同時解はサポートせず、**経路A と経路B を別ランで比較する運用前提**（`physics_mode` を切り替えた個別ラン＋extended_diagnostics でログ）。[marsdisk/run_zero_d.py#run_zero_d][docs/devnotes/phase7_minimal_diagnostics.md]
