...
火星の衛星フォボス・ダイモスの起源は、巨大衝突による火星周回円盤から形成されたとする説が有力です（Rosenblatt 2011, [@CanupSalmon2018_SciAdv4_eaar6887]）。本シミュレーションは、ロッシュ限界（~2.4 火星半径）内に形成された高温・高密度ダスト円盤の時間進化を追跡します。
...
- [@TakeuchiLin2003_ApJ593_524] は既定で無効（`ALLOW_TL2003=false`）
...
[@Hyodo2017a_ApJ845_125], 2018) が報告する蒸気分率≲数 % の gas-poor 円盤を first-order 近似するため、既定では P_gas=0 を採用しています。
...
**最大残存率** ([@LeinhardtStewart2012_ApJ745_79]:

$$\large
\frac{M_{LR}}{M_{tot}} \approx 0.5 \left(2 - \frac{Q_R}{Q_{RD}^*}\right)
$$

**破壊閾値 $Q_D^*$** ([@BenzAsphaug1999_Icarus142_5]:
...
- 円盤の初期温度 ~2000 K、蒸気分率≲数 %、メートル級ドロップレットが 1–5 km/s の衝突で再溶融し、∼100 µm 級の液滴へ細分化される、という描像は [@Hyodo2017a_ApJ845_125]I) の SPH 計算と衝突後進化に基づいています。
- 気相からの凝縮で ~0.1 µm スケールの微小ダストが自然に生じること、溶融物から固化した粒子は 0.1–1 mm 程度の粗い粒子になることは [@Ronnet2016_ApJ828_109] の凝縮モデルから取っています。
- T≈2000 K, P≈10⁻⁴ bar 程度の条件で溶融物と蒸気が共存する円盤を想定し、そこからの化学組成・揮発性の扱いを決める、という考え方は [@Pignatale2018_ApJ853_118] に揃えています。
- 放射圧の β の定義と β>0.5 を境界にしたブローアウト条件は、[@Burns1979_Icarus40_1] および [@StrubbeChiang2006_ApJ648_652] に従っており、AU Mic デブリ円盤の「衝突カスケード + 表層外流」モデルと同じスケーリングを採用しています。
- 衝突寿命 `t_coll = 1/(Ω τ_⊥)` の形は、[@Wyatt2008] のデブリ円盤レビューで使われている標準的なスケーリングをそのまま取り入れ、Smoluchowski 解との整合確認用にも使っています。
- gas-poor 円盤を標準とすること、および初期質量の上限を抑えることは、[@Hyodo2017a_ApJ845_125], 2018) の蒸気分率・放射圧評価と、[@CanupSalmon2018_SciAdv4_eaar6887] や [@Kuramoto2024] の起源レビューに整合するように選んでいます。  
...
- [@LeinhardtStewart2012_ApJ745_79]: 破砕強度モデル
- [@StrubbeChiang2006_ApJ648_652]: 放射圧支配の描像
- [@Burns1979_Icarus40_1]: 放射圧 β の定義
- [@Wyatt2008]: 衝突寿命スケーリング
- [@Krivov2006_AA455_509]: Smoluchowski 方程式の実装
- [@Pignatale2018_ApJ853_118]: HKL 昇華モデル
