...
| `make_sweep_summary.py` | temp_supply_sweep バッチ全体のサマリー・ヒートマップ生成。 | sweep バッチディレクトリ（`T*_eps*_tau*` サブディレクトリ群）。 | CSV (`sweep_summary.csv`)、PNG (`fig_sweep_mloss.png`, `fig_sweep_clip.png`, `fig_sweep_sensitivity.png`)。 |
| `plot_mass_loss_map.py` | 1 周期あたり質量損失率ヒートマップ。 | `scripts/sweeps/sweep_mass_loss_map.py` が生成した CSV。 | PNG (`fig_massloss_heatmap.png`)。 |
