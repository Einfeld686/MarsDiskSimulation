...
表 1.2 は、Phase 1–4 の代表的研究を「手法」「入力」「出力」「次段階への受け渡し量」で整理した一覧である。図 1.2a は表の情報の流れ（どの段階の出力が次段階の入力になるか）を一枚に圧縮した図であり、図 1.2b はそのうち Phase 2（Update）の内部を拡大し、本研究が追加する計算（入力更新）の中身を示す。Phase 別の詳細は 2 章で補足する。

**表 1.2：火星巨大衝突円盤研究の役割分担（Phase × 手法 × 入出力）**

| Phase（Role） | 時間スケール | 代表的手法 | 主な入力（計算に与える量） | 主な出力（次へ渡す量） | 代表文献（例） | 本研究との関係 |
|---|---|---|---|---|---|---|
| Phase 1（Input：衝突直後の初期条件） | \(t \lesssim\) 数十時間（\(\sim10^{-3}\)–\(10^{-2}\,\mathrm{yr}\)） | SPH 衝突計算 | 衝突条件（速度・角度・質量比など） | 円盤質量・角運動量、半径方向分布、温度・相（溶融>95%／蒸気<5%）、初期粒径帯（\(\sim1.5\,\mathrm{m}\) 溶融滴）、軌道要素 | Citron et al. (2015); Hyodo et al. (2017a) | Phase 2 の初期値（\(M_{\rm in}, T, a\) など）を与える。ただし数十時間以降の質量推定や外側低密度部は不確かさが残る。 |
| Phase 2（Update：高温期の短期損失） | 年スケール（冷却期） | 熱・粒子過程モデル（個別／部分結合） | Phase 1 の初期状態、火星放射（表面温度履歴） | 蒸気散逸・放射圧除去・昇華による損失可能性、粒径依存の除去条件 | Hyodo et al. (2018); Takeuchi & Lin (2003); Markkanen & Agarwal (2020) | 既存研究は「起こり得る損失」を示すが、\(M_{\rm in}\) を更新して Phase 3 に渡す手続きが未整備。本研究がここを定式化する。 |
| Phase 3（Connection：粘性拡散→外側集積への受け渡し） | \(10^2\)–\(10^6\,\mathrm{yr}\) | 内側：1D 連続円盤＋外側：N 体（ハイブリッド） | 更新後の \(M_{\rm in}\)、外縁 \(r_{\rm d}\)、粘性則 | 外側への供給履歴、衛星の形成・移動（共鳴掃引を含む）、生存条件 | Salmon & Canup (2012); Canup & Salmon (2018); Rosenblatt et al. (2016); Crida & Charnoz (2012); Rosenblatt & Charnoz (2012) | Phase 2 が更新した \(M_{\rm in}'\) を入力として受け取り、形成結果（Phase 4）へ接続する。 |
| Phase 4（Output：観測制約との照合） | Myr–現在 | 潮汐進化・集積史・分光解釈 | Phase 3 の形成結果（質量・軌道）と組成仮説 | 観測との整合性、許される円盤条件の制約 | Rosenblatt (2011); Pignatale et al. (2018); Ronnet et al. (2016); Kuramoto (2024) | 本研究の \(M_{\rm in}'\) と粒径分布は、形成条件の制約と分光解釈の両方に影響し得る。 |
