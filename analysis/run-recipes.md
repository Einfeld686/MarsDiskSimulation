...
- `summary.json` の `s_min_components` に `config`,`blowout`,`effective` が揃い、`psd.floor.mode="none"` のときは `s_min_effective` が `s_min_config` と一致し、それ以外は `max(config, blowout)`（`evolve_smin` は `floor_dynamic` を含む）に一致すること。昇華設定は床粒径へは反映されず、粒径侵食による欠損は `mass_lost_sublimation_step` と `dSigma_dt_sublimation` で診断する。[marsdisk/run_zero_d.py#run_zero_d [L1392–L6000]][marsdisk/run_zero_d.py#run_zero_d [L1392–L6000]][marsdisk/physics/psd.py#apply_uniform_size_drift [L430–L571]]
