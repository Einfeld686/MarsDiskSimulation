...
<!--
NOTE: このファイルは analysis/thesis_sections/introduction/*.md の結合で生成する。
...
\]
ここで \(M_{\rm in}^{\rm SPH}\) は SPH 終端時刻におけるロッシュ限界内側円盤質量であり、\(\Delta M_{\rm in}\) は遷移期に不可逆に失われた分を表す。（要確認：落下分を \(\Delta M_{\rm in}\) に含めるか、別項で扱うかの設計方針）
