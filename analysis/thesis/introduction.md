...
第一に、フォボスとダイモスはいずれも低離心のほぼ円軌道であり、軌道面は火星赤道面に近い。暗い小天体が捕獲された場合、捕獲直後は高離心・高傾斜の軌道になりやすく、現在の軌道に至るには減衰過程が必要だと指摘されてきた（Hyodo et al. 2017a; Glotch et al. 2018）。とくにダイモスは火星から遠く潮汐散逸が弱いため、潮汐進化だけで円軌道化することは難しいとされる（Craddock 2011; Hyodo et al. 2017b）。TODO(REF:phobos_deimos_inclination_refs_v1)
...
物性（密度・空隙率） & 低密度・多孔質を自然に得るかは未確定 & 多孔質な集積体になり得るという解釈がある（Andert et al. 2010）\\
...
巨大衝突起源説の定量化は、衝突直後（概ね数十時間）を扱う数値流体計算と、より長期（およそ \(10^4\text{--}10^7\ \mathrm{yr}\)）を扱う円盤進化・集積モデルの二つの枠組みで進んできた。前者では平滑化粒子流体力学法（Smoothed Particle Hydrodynamics; SPH）などにより放出デブリの分布や角運動量を見積もり、後者ではロッシュ限界内側を連続体円盤、外側を多天体計算として扱う混成モデルが用いられる（例：Salmon & Canup, 2012; Canup & Salmon, 2018）。ただし両者は前提とする円盤状態が大きく異なるため、SPH の出力をそのまま長期モデルへ渡すのは難しい。
...
この点を具体的に扱った先行研究として、**地球—月系**を対象に、ロッシュ限界内側を流体円盤、外側を多天体重力計算で扱う混成モデルを提示した Salmon & Canup (2012) がある。本研究では、火星系を直接議論する文献（Citron 2015; Hyodo 2017a; Canup & Salmon 2018）に加えて、Salmon & Canup (2012) を「内側円盤の質量収支がどの項で閉じているか」「共鳴トルクが内側円盤外縁をどの程度拘束し得るか」を整理するための参照系として用いる。
...
同論文が扱った散逸過程は二つである。まず「熱い気体が集団として流れ出して逃げる現象」である流体力学的散逸（hydrodynamic escape）を扱う。次に「微粒子が光で押し出されて逃げる現象」である放射圧（radiation pressure）を扱う。以下では、本研究の問題設定に関係が深い点だけを整理する。
...
本研究が注目するのは、遷移期にロッシュ限界内側（内側円盤）で生じる不可逆な質量損失である。長期モデルでは \(M_{\rm in}\) がロッシュ限界外側への供給量と供給時間スケールを通じて、最終的な衛星質量分布を左右する。ところが既存の長期モデルでは、内側円盤の質量収支は「惑星への落下」と「粘性拡散による外側への供給」によって記述され、表層からの追加シンク（放射圧ブローアウト、昇華・蒸気散逸など）を明示的に取り入れる例は限られる（Salmon & Canup, 2012; Canup & Salmon, 2018）。TODO(REF:longterm_model_sink_terms_v1)
...
本研究でいう「表層の厚み」は文献で一意に定義されていないため、**衝突カーネルで用いる幾何学的スケールハイト \(H\simeq i\,a\)** を厚みの代理指標として扱う。ここで \(i\) は代表的な軌道傾斜角であり、表層再供給や衝突頻度の見積もりに影響する。暫定的な基準値として \(i_0=0.05\,\rm rad\) を採用するが、根拠は未確定のため感度パラメータとして明示し、時間制約の下で **2 パターン（\(i_0=0.05\) と \(0.10\,\rm rad\)）** に限定して比較する。TODO(REF:i0_default_0p05_origin_v1)
