...
本モデルは gas-poor 条件下の**軸対称ディスク**を対象とし，鉛直方向は面密度へ積分して扱う．半径方向に分割した 1D 計算を基準とし（[@Hyodo2017a_ApJ845_125; @CanupSalmon2018_SciAdv4_eaar6887; @Olofsson2022_MNRAS513_713]），光学的厚さは主に火星視線方向の $\tau_{\rm los}$ を用いる．必要に応じて $\tau_{\rm los}=f_{\rm los}\tau_{\perp}$ から $\tau_{\perp}$ を導出し，表層 ODE の $t_{\rm coll}$ 評価に使う．粒径分布 $n(s)$ をサイズビンで離散化し，Smoluchowski 衝突カスケード（collisional cascade）と表層の放射圧・昇華による流出を同一ループで結合する（[@Dohnanyi1969_JGR74_2531; @Krivov2006_AA455_509; @StrubbeChiang2006_ApJ648_652]）．

- 標準の物理経路では，Smoluchowski 方程式の時間発展を各半径セルで解き，毎ステップの質量収支検査によって数値誤差を監視する．計算順序は図\ref{fig:methods_main_loop}に従う．放射圧〜流出の依存関係のみを抜粋すると ⟨$Q_{\rm pr}$⟩→β→$s_{\rm blow}$→遮蔽Φ→供給→IMEX 更新→外向流束となる．半径方向の粘性拡散は演算子分割で追加可能とする（[@Krivov2006_AA455_509]）．
- 本研究の基準ケースは 1D とし，粘性拡散は感度試験として必要時のみ有効化する．再現実行で保存する情報は付録A，設定→物理対応は付録Bを参照する．
...
本研究は面密度（$\Sigma$）と面密度フラックス（$\dot{\Sigma}$）を局所量として時間発展させ，総質量や総流出率は面積要素で積分して定義する．0D は同じ環状領域を 1-zone とみなし，代表半径を $r=(r_{\rm in}+r_{\rm out})/2$ と置く．このとき環状領域の面積は
...
で与えられ，総質量は $M=A\Sigma$ として換算できる．同様に，表層流出の総質量流出率 $\dot{M}_{\rm out}$ は式\ref{eq:mdot_out_definition}で定義する．
...
- **放射圧ブローアウト**: β と $s_{\rm blow}$ は式\ref{eq:beta_definition}–\ref{eq:s_blow_definition}で定義し，表層流出（面密度フラックス）は式\ref{eq:surface_outflux}，円盤全体の質量流出率は式\ref{eq:mdot_out_definition}で定義する．
...
7. 遮蔽係数 $\Phi$ を適用して $\kappa_{\rm eff}$ と $\Sigma_{\tau_{\perp}=1}$ を評価する（式\ref{eq:kappa_eff_definition}–\ref{eq:sigma_tau1_definition}）．
