...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RUN_TEMP_SUPPLY_SWEEP_v01 | 温度×供給スイープ（27ケース） | configs/sweep_temp_supply/temp_supply_T4000_eps1.yml | temp_supply_sweep/\<ts\>\_\_\<sha\>\_\_seed\*/T\*\_eps\*\_tau\*/ | T_LIST∈{5000,4000,3000}K × EPS_LIST∈{1.0,0.5,0.1} × TAU_LIST∈{1.0,0.5,0.1} の 27 ケースで τ≈1 維持と供給経路（deep_mixing）を検証する主要スイープ。slab 冷却モデルを使用。 | E.027,E.042,E.043 | FIG_TEMP_SUPPLY_OVERVIEW,FIG_SUPPLY_SURFACE,FIG_OPTICAL_DEPTH | primary | 現在の研究の中心。スイープ結果から M_loss 感度とτ維持条件を評価する。`scripts/research/run_temp_supply_sweep.sh` で実行。 |
