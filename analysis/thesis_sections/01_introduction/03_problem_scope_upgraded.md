...
衝突直後のデブリは非軸対称で、高離心・高傾斜の軌道要素を含むことがある。一方、長期の衛星形成モデルは、時間平均として軸対称で、ほぼ円軌道の薄い円盤を前提に状態量（例：ロッシュ限界内側円盤質量 \(M_{\rm in}\)）を定義する。したがって、数値流体計算（SPH など）の終端から、長期モデルが仮定する準定常円盤に落ち着くまでの時間帯が残る。ここではこれを遷移期と呼ぶ。

遷移期には、位相混合や衝突散逸による円軌道化・赤道化が進む一方で、衝突破砕に伴う粒径分布の変化や、高温条件下での昇華・蒸気散逸なども重なり得る（例：Hyodo et al., 2017a,b; Ronnet et al., 2016）。SPH から長期モデルへの接続を再現可能にするには、遷移期を独立の対象として扱い、そこで生じる不可逆過程を入力更新として取り込む必要がある。

この章で扱う問いは次である。

- 遷移期にロッシュ限界内側（内側円盤）から、長期モデルが明示的に含めない経路で失われる固体質量 \(\Delta M_{\rm in}\) はどれくらいか。
- その損失が、長期モデルへ渡す \(M_{\rm in}\) をどの程度修正するか（\(M_{\rm in}' = M_{\rm in}(t_0) - \Delta M_{\rm in}\)）。

長期モデルでは、内側円盤の物質は「惑星への落下」と「ロッシュ限界外側への拡散（＝外側成長部への供給）」で除去されるものとして質量収支を更新する（Salmon & Canup, 2012; Canup & Salmon, 2018）。一方で、表層からの放射圧ブローアウトや昇華・蒸気散逸といった追加の損失は、長期モデル入力の更新手続きとしては系統的に組み込まれていない（例：Canup & Salmon, 2018）。本研究はこのギャップを埋める。
...
本研究が扱うのは、巨大衝突で形成された火星周回デブリ円盤が、衝突直後の非軸対称・高離心・高傾斜な状態から、長期モデルが仮定する「ほぼ軸対称で円軌道に近い円盤」へ移るまでの遷移期である。衝突直後の円盤は数値流体計算（例：Citron et al., 2015; Hyodo et al., 2017a）が与えるが、長期モデルへ渡すためには、円盤を軸対称な状態量へまとめ直す必要がある（Hyodo et al., 2017b; Canup & Salmon, 2018）。
...
ここで \(\dot{M}_{\rm loss}(t)\) は遷移期の不可逆損失率であり、長期モデルが扱う「惑星への落下」および「ロッシュ限界外への拡散」と二重計上しないよう、放射圧ブローアウトや昇華・蒸気散逸など、遷移期に固有の追加損失に限定して定義する。
...
\(t_{\rm end}\) は「長期モデルが仮定する準定常な円盤に近づき、状態量 \(M_{\rm in}\)・\(r_d\) で表せるようになった時刻」である（Salmon & Canup, 2012; Canup & Salmon, 2018）。ただし、円軌道化・赤道化がどれだけ速いかは円盤の密度や衝突頻度に依存し、遷移期の長さは一意に決まらない（Hyodo et al., 2017b）。本研究ではこの不確実さを隠さず、\(t_{\rm end}\) を入力として明示し、\(\Delta M_{\rm in}\) の \(t_{\rm end}\) 依存性を感度として評価する。
...
本研究でいう「表層の厚み」は文献で一意に定義されていないため、衝突頻度の見積もりに使う幾何学的スケールハイトとして \(H\simeq i\,a\) を導入し、厚みの代理指標として扱う。ここで \(i\) は代表的な軌道傾斜角である。\(i\) は表層再供給や衝突頻度に影響するが、値を一意に固定する根拠は薄い。したがって本研究では \(i\) を感度パラメータとして明示し、時間制約の下で 2 パターン（\(i_0=0.05\) と \(0.10\,\rm rad\)）に限定して比較する。
