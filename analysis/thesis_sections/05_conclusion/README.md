# thesis/conclusion.md 分割ソース

このディレクトリの `*.md` は `analysis/thesis/conclusion.md` の編集用ソースです．
番号付きの分割ファイル（`[0-9][0-9]_*.md`）は結合対象として Git 管理します．
`analysis/thesis_sections/05_conclusion/manifest.txt` がある場合はその順序で結合し，なければ番号順で結合します．
