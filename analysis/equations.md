...
\end{equation}
```
with the component sizes assembled as `s_min_components = {"config": s_{\min,\mathrm{cfg}}, "blowout": s_{\mathrm{blow,raw}}, "blowout_raw": s_{\mathrm{blow,raw}}, "blowout_effective": s_{\mathrm{blow,eff}}, "effective": s_{\min,\mathrm{eff}}}` where `s_{\mathrm{blow,eff}}=\max(s_{\min,\mathrm{cfg}}, s_{\mathrm{blow,raw}})` (`[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`). The sublimation helper `fragments.s_sub_boundary` (`[marsdisk/physics/fragments.py#s_sub_boundary [L160–L226]]`) is now used exclusively for the grain-size evolution hook (``ds/dt``) and no longer raises the PSD floor.
...
Both fields are written to the time series と `summary.json` に `beta_at_smin_config` / `beta_at_smin_effective` として保存される（`[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`）。
The blow-out threshold originates from the module constant `BLOWOUT_BETA_THRESHOLD` (`[marsdisk/physics/radiation.py#BLOWOUT_BETA_THRESHOLD [L58]]`), and the recorded key is `beta_threshold` (`[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`).
The closed-form expression for β is persisted verbatim in `run_config.json["beta_formula"]` as part of the provenance record (`[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`).

Case classification follows the configuration beta: `case_status = "blowout"` when `beta_at_smin_config >= beta_threshold`, otherwise `"ok"`; exceptional mass-budget failures are escalated separately (`[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`). This logic matches the recorded summaries used by downstream validation.

**Recorded quantities**

| Quantity (units) | Summary key(s) | Provenance | Notes |
| --- | --- | --- | --- |
| Effective minimum grain size (m) | `s_min_effective`, `s_min_components["effective"]` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | `\max(s_{\min,\mathrm{cfg}}, s_{\mathrm{blow,eff}})` |
| Configured minimum grain size (m) | `s_min_config`, `s_min_components["config"]` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | YAML `sizes.s_min` from `schema.Sizes` |
| Blow-out limit (raw, m) | `s_blow_m`, `s_min_components["blowout_raw"]`, `s_min_components["blowout"]` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Uses `radiation.blowout_radius` without size-floor clipping |
| Blow-out limit (effective, m) | `s_blow_m_effective`, `s_min_components["blowout_effective"]` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Size-floor clipped value `\max(s_{\min,\mathrm{cfg}}, s_{\mathrm{blow,raw}})` |
| Beta at config size (dimensionless) | `beta_at_smin_config` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Evaluated with `radiation.beta` |
| Beta at effective size (dimensionless) | `beta_at_smin_effective` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Uses same β function at `s_min_effective` |
| Beta threshold (dimensionless) | `beta_threshold` | `[marsdisk/physics/radiation.py#BLOWOUT_BETA_THRESHOLD [L58]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Constant 0.5 defined in radiation module |
| Blow-out loss cumulative (M_Mars) | `M_out_cum` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Equal to `mass_lost_by_blowout` final value |
| Sink loss cumulative (M_Mars) | `M_sink_cum` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Includes HKL erosion and IMEX sink flux |
| Mean blow-out per orbit (M_Mars) | `M_out_mean_per_orbit` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Reported when at least one orbit completes |
| Mean sink per orbit (M_Mars) | `M_sink_mean_per_orbit` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Reported when at least one orbit completes |
| Orbit counter (dimensionless) | `orbits_completed` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Number of completed orbital rollups |
| Mars-facing temperature (K) | `T_M_used`, `T_M_source` | `[marsdisk/run_zero_d.py#load_config [L1334–L1356]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | `radiation.TM_K` または温度ドライバから決定（legacy `temps.T_M` は廃止） |
| Radiation efficiency (dimensionless) | `Q_pr_used` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]`, `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Planck mean stored for reference |
| Shielding Φ table path | `phi_table_path` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]` | Summary retains the resolved τ-table for provenance |
| PSD bin index (dimensionless) | `bin_index` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]][marsdisk/io/writer.py#write_parquet [L412–L438]]` | Time×bin identifier stored in `psd_hist.parquet` |
| PSD bin centre (m) | `s_bin_center` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]][marsdisk/io/writer.py#write_parquet [L412–L438]]` | Logarithmic midpoint used for PSD plots |
| PSD bin number density | `N_bin` | `[marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]][marsdisk/io/writer.py#write_parquet [L412–L438]]` | Relative number surface density per bin |
...
- `sub_params.mass_conserving=true` の場合、昇華由来の ds/dt は粒径のみを縮小し、1 ステップ内に $s<a_{\rm blow}$ を跨いだ分だけをブローアウト損失として処理する（質量シンクには入れない）。false で従来どおり昇華シンクとして質量減算。[marsdisk/physics/collisions_smol.py#step_collisions_smol_0d [L1113–L1571]][marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]
...
**Numerics**
- Consistency check performed each step; thresholds at $10^{-12}$ (warning) and $10^{-6}$ (error). [marsdisk/run_zero_d.py#run_zero_d [L1392–L5921]]
