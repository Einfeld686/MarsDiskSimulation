...
- 式と参照：$⟨Q_{\mathrm{pr}}(s,T_\mathrm{M})⟩ = \mathrm{table.interp}(s,T_\mathrm{M})$（[marsdisk/io/tables.py#interp_qpr [L289–L300]]）
...
- 式と参照：$⟨Q_{\mathrm{pr}}⟩_{\mathrm{interp}} = \text{QPrTable.from\_frame}(\text{read}(path))$（[marsdisk/io/tables.py#load_qpr_table [L313–L325]]）
...
**Numerics**
- Pure algebraic evaluation with argument validation; raises `MarsDiskError` when $\tau_{\perp}\le0$ or $\Omega\le0$. [marsdisk/physics/surface.py#wyatt_tcoll_S1 [L74–L85]]
...
- Indicator $I_{\mathrm{blow}}$ is 1 only when the runtime flag `enable_blowout` is true, ensuring both the $1/t_{\mathrm{blow}}$ termと外向流束が無効化される。[marsdisk/physics/surface.py#step_surface_density_S1 [L108–L184]]

`surface.collision_solver="surface_ode"` を選んだ場合にのみ呼び出され、`t_{\mathrm{sink}}` が `None` または非正（`sinks.mode="none"` のときなど）ならシンク指標を落として吹き飛びと Wyatt/Strubbe–Chiang 型衝突寿命のみを損失項に残す。[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]][marsdisk/physics/sinks.py#total_sink_timescale [L83–L160]][marsdisk/physics/surface.py#step_surface_density_S1 [L108–L184]] その構成では `sink_flux` は常に0となる。
...
\end{equation}
```
with the component sizes assembled as `s_min_components = {"config": s_{\min,\mathrm{cfg}}, "blowout": s_{\mathrm{blow}}, "effective": s_{\min,\mathrm{eff}}}` (`[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`). The sublimation helper `fragments.s_sub_boundary` (`[marsdisk/physics/fragments.py#s_sub_boundary [L148–L214]]`) is now used exclusively for the grain-size evolution hook (``ds/dt``) and no longer raises the PSD floor.
...
```
where `radiation.beta` implements the Stefan–Boltzmann expression and takes the Planck mean `⟨Q_{\mathrm{pr}}⟩` from the current run (`[marsdisk/physics/radiation.py#beta [L438–L459]]`).
Both fields are written to the time series と `summary.json` に `beta_at_smin_config` / `beta_at_smin_effective` として保存される（`[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`）。
The blow-out threshold originates from the module constant `BLOWOUT_BETA_THRESHOLD` (`[marsdisk/physics/radiation.py#BLOWOUT_BETA_THRESHOLD [L54]]`), and the recorded key is `beta_threshold` (`[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`).
The closed-form expression for β is persisted verbatim in `run_config.json["beta_formula"]` as part of the provenance record (`[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`).

Case classification follows the configuration beta: `case_status = "blowout"` when `beta_at_smin_config >= beta_threshold`, otherwise `"ok"`; exceptional mass-budget failures are escalated separately (`[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`). This logic matches the recorded summaries used by downstream validation.

**Recorded quantities**

| Quantity (units) | Summary key(s) | Provenance | Notes |
| --- | --- | --- | --- |
| Effective minimum grain size (m) | `s_min_effective`, `s_min_components["effective"]` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | `\max(s_{\min,\mathrm{cfg}}, s_{\mathrm{blow}})` |
| Configured minimum grain size (m) | `s_min_config`, `s_min_components["config"]` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | YAML `sizes.s_min` from `schema.Sizes` |
| Blow-out limit (m) | `s_blow_m`, `s_min_components["blowout"]` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Uses `radiation.blowout_radius` |
| Beta at config size (dimensionless) | `beta_at_smin_config` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Evaluated with `radiation.beta` |
| Beta at effective size (dimensionless) | `beta_at_smin_effective` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Uses same β function at `s_min_effective` |
| Beta threshold (dimensionless) | `beta_threshold` | `[marsdisk/physics/radiation.py#BLOWOUT_BETA_THRESHOLD [L54]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Constant 0.5 defined in radiation module |
| Blow-out loss cumulative (M_Mars) | `M_out_cum` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Equal to `mass_lost_by_blowout` final value |
| Sink loss cumulative (M_Mars) | `M_sink_cum` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Includes HKL erosion and IMEX sink flux |
| Mean blow-out per orbit (M_Mars) | `M_out_mean_per_orbit` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Reported when at least one orbit completes |
| Mean sink per orbit (M_Mars) | `M_sink_mean_per_orbit` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Reported when at least one orbit completes |
| Orbit counter (dimensionless) | `orbits_completed` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Number of completed orbital rollups |
| Mars-facing temperature (K) | `T_M_used`, `T_M_source` | `[marsdisk/run_zero_d.py#load_config [L212–L234]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | `radiation.TM_K` または温度ドライバから決定（legacy `temps.T_M` は廃止） |
| Radiation efficiency (dimensionless) | `Q_pr_used` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]`, `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Planck mean stored for reference |
| Shielding Φ table path | `phi_table_path` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]` | Summary retains the resolved τ-table for provenance |
| PSD bin index (dimensionless) | `bin_index` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]][marsdisk/io/writer.py#write_parquet [L24–L391]]` | Time×bin identifier stored in `psd_hist.parquet` |
| PSD bin centre (m) | `s_bin_center` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]][marsdisk/io/writer.py#write_parquet [L24–L391]]` | Logarithmic midpoint used for PSD plots |
| PSD bin number density | `N_bin` | `[marsdisk/run_zero_d.py#run_zero_d [L270–L4910]][marsdisk/io/writer.py#write_parquet [L24–L391]]` | Relative number surface density per bin |
...
**Numerics**
- Direct multiplication with argument validation; raises `MarsDiskError` when $\Omega\le0$. [marsdisk/physics/surface.py#compute_surface_outflux [L209–L218]]
...
- Smol が既定の衝突経路（`surface.collision_solver="smol"`）。表層 ODE (S1) を試す場合のみ `surface_ode` へ切り替える。[marsdisk/io/diagnostics.py#write_zero_d_history [L28–L128]]
...
- Absolute error used to avoid cancellation sign issues; logs diagnostic values. [marsdisk/physics/smol.py#compute_mass_budget_error_C4 [L409–L473]]
...
- Lookup arguments clamped to tabulated ranges within `qpr_lookup`; fallback to unity when no table available. [marsdisk/physics/radiation.py#planck_mean_qpr [L424–L435]]
...
- No additional clamps beyond those in the helper routines. [marsdisk/physics/radiation.py#beta [L438–L459]]
...
- No iteration; direct algebraic evaluation for $\beta=0.5$ threshold. [marsdisk/physics/radiation.py#blowout_radius [L462–L486]]
...
**Numerics**
- Returns infinity when the opacity is non-positive to signal no optical-depth limit; type validation ensures real input. [marsdisk/physics/shielding.py#sigma_tau1 [L124–L131]]
...
- Wraps the interpolator to reuse `effective_kappa` clamping of $\Phi$ to $[0,1]$. [marsdisk/physics/shielding.py#apply_shielding [L134–L217]]
...
- Ambient vapour pressure $P_{\mathrm{gas}}$ (特に Si を含む蒸気分圧) は Ronnet et al. (2016) 同様に自由パラメータとして扱い、化学平衡は計算しない。既定は gas‑poor 前提で $P_{\mathrm{gas}}=0$ とし、感度試験では YAML `sinks.sub_params.P_gas` を明示調整すること（HKL フラックスの最大不確定要素）。[marsdisk/physics/sublimation.py#p_sat [L588–L603]], [marsdisk/schema.py#Supply [L446–L552]]
- `sub_params.mass_conserving=true` の場合、昇華由来の ds/dt は粒径のみを縮小し、1 ステップ内に $s<a_{\rm blow}$ を跨いだ分だけをブローアウト損失として処理する（質量シンクには入れない）。false で従来どおり昇華シンクとして質量減算。[marsdisk/physics/collisions_smol.py#step_collisions_smol_0d [L757–L1188]][marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]
...
- See also [marsdisk/physics/sublimation.py#sublimation_sink_from_dsdt [L684–L720]] for per-bin sink conversion.
...
- Clamps the denominator to avoid division by zero for $e \to 1$. [marsdisk/physics/dynamics.py#v_rel_pericenter [L49–L59]]
...
- Logs iteration progress for traceability. [marsdisk/physics/dynamics.py#solve_c_eq [L62–L121]]
...
- Logs the before/after eccentricities for debugging. [marsdisk/physics/dynamics.py#update_e [L124–L155]]
...
- Returns a Python closure that evaluates $\Sigma(r)$ for downstream use. [marsdisk/physics/initfields.py#sigma_from_Minner [L19–L46]]
...
- Emits diagnostic logs with the number of size bins. [marsdisk/physics/collide.py#compute_collision_kernel_C1 [L66–L163]]
...
- When clipping is active, enforces non-negative densities. [marsdisk/physics/initfields.py#surf_sigma_init [L49–L81]]
...
- Helper `_q_d_star` carries out the power-law evaluation with the active `coeff_units` applied; gravity scaling uses `_gravity_velocity_scale` with the configured `mu_grav`. [marsdisk/physics/qstar.py#compute_q_d_star_F1 [L288–L340]]
...
- Ensures non-negative output by clipping with `max`. [marsdisk/physics/supply.py#get_prod_area_rate [L497–L501]]
...
- Leaves the global shielding state untouched—callers retain the returned callable. [marsdisk/physics/shielding.py#load_phi_table [L53–L68]]
...
- Operates on scalars and returns a float suitable for downstream arrays. [marsdisk/physics/shielding.py#clip_to_tau1 [L220–L262]]
...
- Output is explicitly coerced to `float` for consistency with NumPy inputs. [marsdisk/physics/fragments.py#compute_q_r_F2 [L79–L108]]
...
- Raises `MarsDiskError` for non-positive `q_rd_star` to avoid division-by-zero. [marsdisk/physics/fragments.py#compute_largest_remnant_mass_fraction_F2 [L111–L145]]
...
```
The helper expects square matrices and warns via `MarsDiskError` on shape mismatches. [marsdisk/physics/collide.py#compute_prod_subblow_area_rate_C2 [L166–L205]]
...
- Serves as the baseline branch for HKL mass-flux calculations. [marsdisk/physics/sublimation.py#p_sat_clausius [L186–L204]]
...
Interpolates $\log_{10}P_{\mathrm{sat}}(T)$ using a shape-preserving cubic (PCHIP) constructed from the loaded table. Temperatures outside the tabulated span are clipped with a logged warning; the interpolant guarantees monotonic pressure. [marsdisk/physics/sublimation.py#p_sat_tabulated [L415–L429]]
...
- ログに選択ブランチを記録。 [marsdisk/physics/sublimation.py#p_sat [L588–L603]]
...
s_{\mathrm{sink}} = \frac{\eta_{\mathrm{instant}}\,t_{\mathrm{ref}}\,J(T)}{\rho}.
```
See (E.019) for symbol definitions; this helper mainly performs validation and delegates flux evaluation. [marsdisk/physics/sublimation.py#s_sink_from_timescale [L665–L681]]
...
Loads a Planck-mean $⟨Q_{\mathrm{pr}}⟩$ table into the radiation module cache, logging the covered size and temperature ranges when available. The returned callable defaults subsequent lookups via `qpr_lookup`. [marsdisk/physics/radiation.py#load_qpr_table [L223–L251]]
...
### (E.044) marsdisk/physics/collisions_smol.py: kernel_minimum_tcoll (lines 444–463)
[@Wyatt2008]
...
### (E.045) marsdisk/physics/collisions_smol.py: supply_mass_rate_to_number_source (lines 174–272)
[@Birnstiel2011_AA525_A11; @Krivov2006_AA455_509]

質量フラックス $\dot{M}_{\rm supply}$ (kg s$^{-1}$) をビン別個数ソース $F_k$ (s$^{-1}$) に変換する。既定では $s \ge s_{\rm min,eff}$ を満たす最小ビンへ集中注入し、質量保存条件
```latex
\sum_{k} m_k F_k = \dot{M}_{\mathrm{supply}}
```
が成り立つよう $F_{k_{\rm inj}} = \dot{M}_{\rm supply} / m_{k_{\rm inj}}$ とする。負のフラックスはゼロとして扱う。

`mode="powerlaw_bins"` を選ぶと、ビン幅 `widths` を用いて $[s_{\rm inj,min}, s_{\rm inj,max}]$ に $dN/ds\propto s^{-q}$ を割り当てる。重み $w_k$ をサイズ区間の重み付き積分として計算し、$F_k = w_k\,\dot{M}_{\rm supply}/m_k$ でスケールする（$w_k \le 0$ なら最小ビン集中注入へフォールバック）。

> **注記**: この集中注入＋パワー法分配は、セクショナル Smoluchowski 解法で質量保存を保ったまま注入幅を変える標準的な近似である。Numba が利用可能なら powerlaw 経路も JIT で高速化される。
...
|$q$|Power-law slope|—|`supply_q`（既定 3.5）|
|$s_{\rm inj,min}, s_{\rm inj,max}$|Injection bounds|m|省略時は自動で $(s_{\min,\mathrm{eff}}, \max s)$|
|`widths`|Bin widths|m|powerlaw 分配時は必須|

---

### (E.045a) marsdisk/physics/collide.py: compute_collision_kernel_bookkeeping (lines 208–353)
[@Thebault2003_AA410_685; @Krivov2006_AA455_509]

エネルギー簿記付き衝突カーネルを返す (C1 拡張)。衝突レート $C_{ij}$ に基づき、相対運動エネルギー $E_{\mathrm{rel}}=\tfrac12\mu_{ij}v_{ij}^2$ を積分して散逸率を評価する。
```latex
\begin{aligned}
E_{\mathrm{rel,step}} &= \sum_{i\le j} C_{ij}\,E_{\mathrm{rel},ij},\\
E_{\mathrm{ret,step}} &= \sum_{i\le j} C_{ij}\,f_{\mathrm{ke},ij}\,E_{\mathrm{rel},ij},\\
E_{\mathrm{diss,step}} &= E_{\mathrm{rel,step}} - E_{\mathrm{ret,step}},\\
f_{\mathrm{ke,mean}} &= \frac{\sum_{i\le j} C_{ij} f_{\mathrm{ke},ij}}{\sum_{i\le j} C_{ij}},\quad
f_{\mathrm{ke,energy}} = \frac{E_{\mathrm{ret,step}}}{E_{\mathrm{rel,step}}},\\
...
|Symbol|Meaning|Units|Defaults/Notes|
|---|---|---|---|
|$C_{ij}$|Collision kernel|s$^{-1}$|C1 相当、Numba 可|
...
### (E.046) marsdisk/physics/collisions_smol.py: step_collisions_smol_0d (lines 678–1098)
[@Wyatt2008; @Thebault2003_AA410_685; @Krivov2006_AA455_509]
...
- **破片分布**: $Y_{kij}$ の重みは質量分布 $dM/ds\propto s^{-\alpha_{\rm frag}}$ をビン端点で積分し、$w_k \propto \int_{s_{k-}}^{s_{k+}} s^{-\alpha_{\rm frag}} ds$ を正規化して構成する。
- **遮蔽 headroom**: $\Sigma_{\tau=1}$ を超える供給は `headroom_policy="clip"` でカットし、`"spill"` で過剰分を後縮尺する。
...
|$K_{ij}$|Collision kernel|m$^3$ s$^{-1}$|`compute_collision_kernel_C1` または (E.045a)|
...
|$F_k$|Source rate|s$^{-1}$|`supply_mass_rate_to_number_source`（集中/冪分配）|
|`headroom_policy`|Supply gating mode|—|`clip` でカット、`spill` で縮尺|
...
### (E.047) marsdisk/physics/collide.py: compute_collision_kernel_bookkeeping (lines 208–353)

衝突ペア $(i,j)$ の相対運動エネルギーを[@Thebault2003_AA410_685; @Krivov2006_AA455_509]
```latex
E_{\mathrm{rel},ij} = \frac{1}{2}\,\mu_{ij}\,v_{ij}^2,\qquad
\mu_{ij} = \frac{m_i m_j}{m_i + m_j}
```
で評価する。Reduced mass $\mu_{ij}$ と速度 $v_{ij}$ は上三角のみを用いて二重カウントを避ける。

**Symbols**

|Symbol|Meaning|Units|Defaults/Notes|
|---|---|---|---|
|$m_i, m_j$|Bin masses|kg|PSD stateから取得|
|$v_{ij}$|Relative velocity|m s$^{-1}$|スカラーまたは行列入力|
|$\mu_{ij}$|Reduced mass|kg|$m_i m_j/(m_i+m_j)$|
|$E_{\mathrm{rel},ij}$|Pairwise relative kinetic energy|J|上三角のみ評価|

**Numerics**
- Uses upper-triangular summation to avoid double counting; [marsdisk/physics/collide.py#compute_collision_kernel_bookkeeping [L208–L353]]

---

### (E.048) marsdisk/physics/collide.py: compute_collision_kernel_bookkeeping (lines 208–353)

相対エネルギーのステップ集計は上三角和[@Thebault2003_AA410_685; @Krivov2006_AA455_509]
```latex
E_{\mathrm{rel,step}} = \sum_{i\le j} C_{ij}\,E_{\mathrm{rel},ij}
```
で計算し、`E_rel_step` 列として記録する。$C_{ij}$ は (E.024) に準じる衝突レート（対称）を用いる。

**Symbols**

|Symbol|Meaning|Units|Defaults/Notes|
|---|---|---|---|
|$C_{ij}$|Collision kernel|s$^{-1}$|上三角のみ和を取る|
|$E_{\mathrm{rel,step}}$|Rate-weighted relative energy|J m$^{-2}$ s$^{-1}$|`energy_stats[0]`|

**Numerics**
- Collision-rate weighted sum over upper triangle. [marsdisk/physics/collide.py#compute_collision_kernel_bookkeeping [L208–L353]]

---

### (E.049) marsdisk/physics/collide.py: compute_collision_kernel_bookkeeping (lines 208–353)

非散逸率 $f_{\mathrm{ke},ij}$ に基づく残存／散逸エネルギーは[@Thebault2003_AA410_685; @Krivov2006_AA455_509]
```latex
E_{\mathrm{ret,step}} = \sum_{i\le j} C_{ij}\,f_{\mathrm{ke},ij}\,E_{\mathrm{rel},ij},\qquad
E_{\mathrm{diss,step}} = E_{\mathrm{rel,step}} - E_{\mathrm{ret,step}}
```
として求め、`E_retained_step` と `E_dissipated_step` 列に対応する。$f_{\mathrm{ke},ij}$ は $F_{\mathrm{lf}}>0.5$ で `f_ke_cratering`、それ以外で `f_ke_fragmentation`（未指定時は $\varepsilon^2$）を適用する。

**Symbols**

|Symbol|Meaning|Units|Defaults/Notes|
|---|---|---|---|
|$f_{\mathrm{ke},ij}$|Non-dissipated kinetic-energy fraction|—|侵食/破砕で分岐|
|$E_{\mathrm{ret,step}}$|Retained kinetic energy|J m$^{-2}$ s$^{-1}$|`energy_stats[2]`|
|$E_{\mathrm{diss,step}}$|Dissipated energy|J m$^{-2}$ s$^{-1}$|`energy_stats[1]`|

**Numerics**
- Fractional energy dissipation depends on regime (cratering vs fragmentation). [marsdisk/physics/collide.py#compute_collision_kernel_bookkeeping [L208–L353]]

---

### (E.050) marsdisk/physics/collide.py: compute_collision_kernel_bookkeeping (lines 208–353)

エネルギー平均と設定不一致の指標[@Thebault2003_AA410_685; @Krivov2006_AA455_509]:
```latex
\bar{f}_{\mathrm{ke}}^{(C)} = \frac{\sum_{i\le j} C_{ij} f_{\mathrm{ke},ij}}{\sum_{i\le j} C_{ij}},\quad
\bar{f}_{\mathrm{ke}}^{(E)} = \frac{E_{\mathrm{ret,step}}}{E_{\mathrm{rel,step}}},\quad
\Delta f_{\mathrm{ke},\varepsilon} = \left| f_{\mathrm{ke,frag}} - \varepsilon^2 \right|.
```
`f_ke_mean` は $\bar{f}_{\mathrm{ke}}^{(C)}$, `f_ke_energy` は $\bar{f}_{\mathrm{ke}}^{(E)}$ に対応する。$f_{\mathrm{ke,frag}}$ を明示すると $\Delta f_{\mathrm{ke},\varepsilon}>0.1$ で警告を出し、`f_ke_eps_mismatch` 列に差分を残す。

**Symbols**

|Symbol|Meaning|Units|Defaults/Notes|
|---|---|---|---|
|$\bar{f}_{\mathrm{ke}}^{(C)}$|Collision-rate weighted mean|—|`f_ke_mean`|
|$\bar{f}_{\mathrm{ke}}^{(E)}$|Energy-weighted mean|—|`f_ke_energy`|
|$f_{\mathrm{ke,frag}}$|Fragmentation-side $f_{\mathrm{ke}}$|—|未指定時は $\varepsilon^2$|
|$\varepsilon$|Coefficient of restitution|—|`eps_restitution`|
|$\Delta f_{\mathrm{ke},\varepsilon}$|Mismatch indicator|—|`f_ke_eps_mismatch`|

**Numerics**
- Weighted averages computed over collision kernel; mismatch flagged if > 0.1. [marsdisk/physics/collide.py#compute_collision_kernel_bookkeeping [L208–L353]]

---

### (E.051) marsdisk/physics/collide.py: compute_collision_kernel_bookkeeping (lines 208–353)

最大残骸分率 $F_{\mathrm{lf}}$ に基づく分岐診断[@Thebault2003_AA410_685; @Krivov2006_AA455_509]:
```latex
...
**Numerics**
- Categorizes collision pairs by largest remnant fraction. [marsdisk/physics/collide.py#compute_collision_kernel_bookkeeping [L208–L353]][marsdisk/physics/fragments.py#compute_largest_remnant_mass_fraction_F2 [L111–L145]]

---

### (E.052) marsdisk/run_zero_d.py: energy bookkeeping consistency (lines 2558–2594)

エネルギー簿記の内部整合性は[@Thebault2003_AA410_685]
```latex
...
**Numerics**
- Consistency check performed each step; thresholds at $10^{-12}$ (warning) and $10^{-6}$ (error). [marsdisk/run_zero_d.py#run_zero_d [L270–L4910]]

---

### (E.053) marsdisk/run_zero_d.py: _surface_energy_floor (lines 139–160)

Krijt & Kama 型の表面エネルギー制約による最小粒径[@KrijtKama2014_AA566_L2]
```latex
s_{\min,\mathrm{surf}} = \left[ \frac{6\gamma(\alpha - 3)}{\eta\,\rho\,v_{\mathrm{rel}}^2} \right]^2 s_{\max}^{\alpha - 5},\qquad (\alpha \ne 5)
```
で評価し、$\alpha\le3$ や非物理入力では 0 とし、$s_{\min,\mathrm{surf}}>s_{\max}$ は $s_{\max}$ にクリップする。`s_min_surface_energy` として `series/run.parquet` に出力し、`s_min_effective` の候補に加える。
...
|$s_{\max}$|Maximum grain size|m|`sizes.s_max`|
|$s_{\min,\mathrm{surf}}$|Surface-energy-limited minimum size|m|`s_min_surface_energy`|

**Numerics**
- Clamps result to $[0, s_{\max}]$; returns 0 for unphysical inputs. [marsdisk/run_zero_d.py#_surface_energy_floor [L150–L171]]
