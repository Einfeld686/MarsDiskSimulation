...
- 式と参照：$⟨Q_{\mathrm{pr}}(s,T_\mathrm{M})⟩ = \mathrm{table.interp}(s,T_\mathrm{M})$（[marsdisk/io/tables.py#interp_qpr [L366–L377]]）
...
- 式と参照：$⟨Q_{\mathrm{pr}}⟩_{\mathrm{interp}} = \text{QPrTable.from\_frame}(\text{read}(path))$（[marsdisk/io/tables.py#load_qpr_table [L390–L402]]）
...
\end{equation}
```
with the component sizes assembled as `s_min_components = {"config": s_{\min,\mathrm{cfg}}, "blowout": s_{\mathrm{blow,raw}}, "blowout_raw": s_{\mathrm{blow,raw}}, "blowout_effective": s_{\mathrm{blow,eff}}, "effective": s_{\min,\mathrm{eff}}}` where `s_{\mathrm{blow,eff}}=\max(s_{\min,\mathrm{cfg}}, s_{\mathrm{blow,raw}})` (`[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`). The sublimation helper `fragments.s_sub_boundary` (`[marsdisk/physics/fragments.py#s_sub_boundary [L160–L226]]`) is now used exclusively for the grain-size evolution hook (``ds/dt``) and no longer raises the PSD floor.
...
Both fields are written to the time series と `summary.json` に `beta_at_smin_config` / `beta_at_smin_effective` として保存される（`[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`）。
The blow-out threshold originates from the module constant `BLOWOUT_BETA_THRESHOLD` (`[marsdisk/physics/radiation.py#BLOWOUT_BETA_THRESHOLD [L58]]`), and the recorded key is `beta_threshold` (`[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`).
The closed-form expression for β is persisted verbatim in `run_config.json["beta_formula"]` as part of the provenance record (`[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`).

Case classification follows the configuration beta: `case_status = "blowout"` when `beta_at_smin_config >= beta_threshold`, otherwise `"ok"`; exceptional mass-budget failures are escalated separately (`[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`). This logic matches the recorded summaries used by downstream validation.

**Recorded quantities**

| Quantity (units) | Summary key(s) | Provenance | Notes |
| --- | --- | --- | --- |
| Effective minimum grain size (m) | `s_min_effective`, `s_min_components["effective"]` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | `\max(s_{\min,\mathrm{cfg}}, s_{\mathrm{blow,eff}})` |
| Configured minimum grain size (m) | `s_min_config`, `s_min_components["config"]` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | YAML `sizes.s_min` from `schema.Sizes` |
| Blow-out limit (raw, m) | `s_blow_m`, `s_min_components["blowout_raw"]`, `s_min_components["blowout"]` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Uses `radiation.blowout_radius` without size-floor clipping |
| Blow-out limit (effective, m) | `s_blow_m_effective`, `s_min_components["blowout_effective"]` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Size-floor clipped value `\max(s_{\min,\mathrm{cfg}}, s_{\mathrm{blow,raw}})` |
| Beta at config size (dimensionless) | `beta_at_smin_config` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Evaluated with `radiation.beta` |
| Beta at effective size (dimensionless) | `beta_at_smin_effective` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Uses same β function at `s_min_effective` |
| Beta threshold (dimensionless) | `beta_threshold` | `[marsdisk/physics/radiation.py#BLOWOUT_BETA_THRESHOLD [L58]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Constant 0.5 defined in radiation module |
| Blow-out loss cumulative (M_Mars) | `M_out_cum` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Equal to `mass_lost_by_blowout` final value |
| Sink loss cumulative (M_Mars) | `M_sink_cum` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Includes HKL erosion and IMEX sink flux |
| Mean blow-out per orbit (M_Mars) | `M_out_mean_per_orbit` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Reported when at least one orbit completes |
| Mean sink per orbit (M_Mars) | `M_sink_mean_per_orbit` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Reported when at least one orbit completes |
| Orbit counter (dimensionless) | `orbits_completed` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Number of completed orbital rollups |
| Mars-facing temperature (K) | `T_M_used`, `T_M_source` | `[marsdisk/run_zero_d.py#load_config [L260–L282]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | `radiation.TM_K` または温度ドライバから決定（legacy `temps.T_M` は廃止） |
| Radiation efficiency (dimensionless) | `Q_pr_used` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]`, `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Planck mean stored for reference |
| Shielding Φ table path | `phi_table_path` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]` | Summary retains the resolved τ-table for provenance |
| PSD bin index (dimensionless) | `bin_index` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]][marsdisk/io/writer.py#write_parquet [L412–L438]]` | Time×bin identifier stored in `psd_hist.parquet` |
| PSD bin centre (m) | `s_bin_center` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]][marsdisk/io/writer.py#write_parquet [L412–L438]]` | Logarithmic midpoint used for PSD plots |
| PSD bin number density | `N_bin` | `[marsdisk/run_zero_d.py#run_zero_d [L318–L5218]][marsdisk/io/writer.py#write_parquet [L412–L438]]` | Relative number surface density per bin |
...
- Absolute error used to avoid cancellation sign issues; logs diagnostic values. [marsdisk/physics/smol.py#compute_mass_budget_error_C4 [L476–L540]]
...
- Lookup arguments clamped to tabulated ranges within `qpr_lookup`; fallback to unity when no table is available unless strict is enabled. [marsdisk/physics/radiation.py#planck_mean_qpr [L444–L455]]
...
- No additional clamps beyond those in the helper routines. [marsdisk/physics/radiation.py#beta [L458–L479]]
...
- When $\langle Q_{\mathrm{pr}}\rangle$ depends on size and is not explicitly specified, solve $\beta(s)=0.5$ numerically (implicit equation $s_{\mathrm{blow}}=C\,Q_{\mathrm{pr}}(s_{\mathrm{blow}})$). The algorithm is not fixed; fixed-point iteration or root finding are acceptable. [marsdisk/physics/radiation.py#blowout_radius [L482–L526]]
- Convergence criteria (default implementation): stop when `abs(s_new - s_old) <= 1e-6 * max(s_new, 1e-30)` or after `max_iter=8`; if `s_new` is non-finite or `<= 0`, abort and return the last finite positive estimate. Alternative solvers should achieve comparable relative tolerance. [marsdisk/physics/radiation.py#blowout_radius [L482–L526]]
- When $\langle Q_{\mathrm{pr}}\rangle$ is provided as a constant, return the closed-form value. [marsdisk/physics/radiation.py#blowout_radius [L482–L526]]
...
- Ambient vapour pressure $P_{\mathrm{gas}}$ (特に Si を含む蒸気分圧) は Ronnet et al. (2016) 同様に自由パラメータとして扱い、化学平衡は計算しない。既定は gas‑poor 前提で $P_{\mathrm{gas}}=0$ とし、感度試験では YAML `sinks.sub_params.P_gas` を明示調整すること（HKL フラックスの最大不確定要素）。[marsdisk/physics/sublimation.py#p_sat [L590–L605]], [marsdisk/schema.py#Supply [L455–L561]]
- `sub_params.mass_conserving=true` の場合、昇華由来の ds/dt は粒径のみを縮小し、1 ステップ内に $s<a_{\rm blow}$ を跨いだ分だけをブローアウト損失として処理する（質量シンクには入れない）。false で従来どおり昇華シンクとして質量減算。[marsdisk/physics/collisions_smol.py#step_collisions_smol_0d [L1079–L1537]][marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]
...
- See also [marsdisk/physics/sublimation.py#sublimation_sink_from_dsdt [L687–L735]] for per-bin sink conversion.
...
- Callers map $c$ to physical velocity dispersion and kernel eccentricity using $v_K$. [marsdisk/physics/collisions_smol.py#compute_kernel_ei_state [L854–L902]]
...
nσv 形式の衝突**率**を離散サイズビンに適用したもので、ガウス厚さによる $1/\sqrt{2\pi}H$ の補正を含む [@Krivov2006_AA455_509]。
...
- Ensures non-negative output by clipping with `max`. [marsdisk/physics/supply.py#get_prod_area_rate [L531–L535]]
- Delegates mode-specific logic to `_rate_basic`. [marsdisk/physics/supply.py#_rate_basic [L200–L227]]
...
s_{\mathrm{sink}} = \frac{\eta_{\mathrm{instant}}\,t_{\mathrm{ref}}\,J(T)}{\rho}.
```
See (E.019) for symbol definitions; this helper mainly performs validation and delegates flux evaluation. [marsdisk/physics/sublimation.py#s_sink_from_timescale [L669–L684]]
...
Loads a Planck-mean $⟨Q_{\mathrm{pr}}⟩$ table into the radiation module cache, logging the covered size and temperature ranges when available. The returned callable defaults subsequent lookups via `qpr_lookup`. [marsdisk/physics/radiation.py#load_qpr_table [L248–L277]]
...
衝突率 $C_{ij}$ の各行合計に対角項 $C_{ii}$ を加えて最大衝突レートを求め、必要に応じて $N_i$ で割って 1 粒子あたりの損失係数に直した上で、その逆数を「最小衝突時間スケール」$t_{\rm coll,min}$ として返す。
```latex
t_{\mathrm{coll,min}} = \frac{1}{\max_i \left(\left(\sum_j C_{ij} + C_{ii}\right) / \max(N_i,10^{-30})\right)}
```
...
|Symbol|Meaning|Units|Defaults/Notes|
|---|---|---|---|
|$C_{ij}$|Collision rate (includes $N_iN_j$)|m$^{-2}$ s$^{-1}$|`compute_collision_kernel_C1` から与える|
...
質量フラックス $\dot{M}_{\rm supply}$ (kg m$^{-2}$ s$^{-1}$) をビン別個数ソース $F_k$ (m$^{-2}$ s$^{-1}$) に変換する。既定では $s \ge s_{\rm min,eff}$ を満たす最小ビンへ集中注入し、質量保存条件
```latex
...
|Symbol|Meaning|Units|Defaults/Notes|
|---|---|---|---|
|$\dot{M}_{\mathrm{supply}}$|Mass supply rate|kg m$^{-2}$ s$^{-1}$|`prod_subblow_mass_rate`|
|$s_{\rm min,eff}$|Effective minimum size|m|Blow-out or config floor|
|$m_k$|Particle mass in bin $k$|kg|PSD state から取得|
|$F_k$|Number source rate in bin $k$|m$^{-2}$ s$^{-1}$|Smol ODE のソース項|
...
|Symbol|Meaning|Units|Defaults/Notes|
|---|---|---|---|
|$C_{ij}$|Collision rate (includes $N_iN_j$)|m$^{-2}$ s$^{-1}$|上三角のみ和を取る|
...
**Numerics**
- Consistency check performed each step; thresholds at $10^{-12}$ (warning) and $10^{-6}$ (error). [marsdisk/run_zero_d.py#run_zero_d [L318–L5218]]
...
- Clamps result to $[0, s_{\max}]$; returns 0 for non-finite intermediate values. [marsdisk/run_zero_d.py#_surface_energy_floor [L183–L219]]
