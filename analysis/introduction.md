...
捕獲起源を成立させるには、(i) 初期のハイパーボリックな相対運動エネルギーを散逸させて重力圏へ束縛し、(ii) その後に観測される「低離心・低傾斜（ほぼ赤道面）」の軌道へ整形する、という二段階の条件を同時に満たす必要がある。Kuramoto (2024) は、単純なガス抵抗捕獲では周回中の減速に十分なガス密度が要求され、さらに濃いガス包絡は捕獲後の急速な内向きスパイラルを招き得ることを整理している。一方で、遭遇の反復を許す一時捕獲（temporary capture）を経由すれば、要求されるガス密度を下げつつプログレード・低傾斜軌道へ遷移し得る可能性も議論されている（Kuramoto 2024）。
したがって、現時点で起源を一意に確定できる状況ではないが、**「軌道が示す力学的規則性」**を最小仮定で再現する観点では、巨大衝突起源が最も成功している枠組みとして扱われることが多い（例：Rosenblatt 2011; Rosenblatt & Charnoz 2012; Canup & Salmon 2018; Kuramoto 2024）。
...
以上を別の言い方で整理すると、巨大衝突説の研究は (1) **SPH による初期条件の確定**（Phase 1）、(2) **高温・非平衡状態から粘性拡散が適用できる準定常円盤へ移る遷移**（本研究が扱う Phase 2）、(3) **準定常円盤の粘性拡散とロッシュ限界越え供給、外側での集積・生存判定**（Phase 3–4）という「三段階＋評価」に分解できる。欠けているのは (2) であり、ここでの短期損失が (3) の入力（とくに $M_{\rm in}$）を系統的に削り得る点が、本研究の問題設定である。

この点について Hyodo et al. (2017b) は、衝突寿命が内側で短くなり得る（数密度と相対速度が大きいため）ことを示しており、粉砕・微粒子供給が内側で優勢になりやすいことを支持する。
...
以後、本稿では Phase 2 の終端（準定常到達）を、力学条件と熱条件の二重基準で定義する。  
準定常到達時刻 $t_{\rm ss}$ は、代表離心率・傾斜角のいずれも十分小さい（例：$\max\{e(t),i(t)\}<\epsilon_{\rm dyn}$; Hyodo et al. 2017b）こと、かつ火星表面温度 $T_{\rm Mars}(t)$ が定義温度 $T_{\rm th}$ を下回る（$T_{\rm Mars}(t)<T_{\rm th}$; Hyodo et al. 2018）ことを同時に満たす最初の時刻とする。  
ここで $\epsilon_{\rm dyn}$ と $T_{\rm th}$ は Phase 2→3 接続の閾値であり、$t<t_{\rm ss}$ を Phase 2、$t\ge t_{\rm ss}$ を Phase 3 の初期条件として扱う。
...
ただしこれは一次近似に基づく推定であり、著者が放射フラックスの温度依存などを含めて時間積分した再計算では、同程度の条件でも冷却が 1–2 桁長い（数十年〜$10^2$ 年規模）可能性が示唆されている（本研究）。
...
### 5.2 temp_supply_sweep_1d の代表図（研究目的への対応）

以下の図は、`out/temp_supply_sweep_1d/20260105-180522__2499a82da__seed111066691/` の 1D スイープから集計したものである。短期損失を「どれだけ失われるか（累積）」「どの過程が律速か（供給と流出の時間結合）」の二軸で可視化し、研究目的（ロッシュ限界内の短期損失の定量化）に直接対応させた。

（図 5.2a）代表ケース（T=3000 K と 5000 K、$\epsilon_{\rm mix}=1.0$、$\tau_0=0.5$）の供給率（$\dot{\Sigma}_{\rm prod}^{(<a_{\rm blow})}$ を半径積分）と表層流出率（$\dot{M}_{\rm out}$ を半径積分）の時間発展。供給と流出の結合が、どの時刻で強く働くかを示す。  
![供給と表層流出の時系列](../figures/temp_supply_sweep_1d/supply_outflow_timeseries.png)

（図 5.2b）累積損失 $M_{\rm loss}$ のパラメータ感度（T, $\epsilon_{\rm mix}$, $\tau_0$）。高温・低遮蔽側で $M_{\rm loss}$ が大きくなる傾向を俯瞰できる。  
![M_loss 感度ヒートマップ](../figures/temp_supply_sweep_1d/mloss_heatmap.png)

---

## 6. 図表管理（別ファイル）

図表の差し込み案とメタデータ管理は `analysis/figures_insertion_notes.md` に移管した。本書は論証の流れを優先し、研究目的に直結する最小限の図のみを本文に含め、それ以外は別途参照する。
## 参考文献
