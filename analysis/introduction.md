...
以下の図は、先行研究を入力・接続・出力の役割に分け、接続の欠損（未解決点）と本研究の位置を示したものである。まず全体像を図 1.2a に示し、欠けたリンクの中身は図 1.2b で拡大する。図中は機能名を短く示し、代表文献は図の下に整理する。
...
flowchart LR
    %% ===== 入力：SPHが与える初期状態 =====
    subgraph IN["入力：巨大衝突直後の内側円盤（SPH）"]
        IN1["初期条件：M(r), T(r), 相, 粒径帯"]
        IN2["内側貯蔵庫：M_in（ロッシュ限界内）"]
    end
    ...
    subgraph OUR["本研究：短期損失で M_in を更新（欠けたリンク）"]
        ...
        SH["遮蔽：放射が届くのは表層のみ"]
        SURF["表層在庫：Σ_surf（損失の舞台）"]
        REP["表層再供給（感度ノブ）"]

        COL["供給：衝突カスケード（微粒子生成）"]
        SUB["変質：高温昇華（粒径縮小／消失）"]
        RAD["除去：放射圧（ブローアウト）"]

        GAS["蒸気：流体脱出（ガス損失）"]
        SUM["累積損失：ΔM_in（固体＋ガス）"]
