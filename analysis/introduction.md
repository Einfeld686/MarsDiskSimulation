...
本節では、先行研究が「何を入力として与え、何を出力として扱ってきたか」を明示して整理する。衝突直後（〜数十時間）の SPH 計算は、半径方向の質量分布・温度・相・粒径帯を出力し、内側円盤の初期条件を与える（Citron et al., 2015; Hyodo et al., 2017; Hyodo et al., 2018）。内側の高密度円盤を粘性で拡散するモデルは、ロッシュ限界通過による外側への供給と内側落下を出力する（Salmon et al., 2010; Crida & Charnoz, 2012）。内側→外側の結び目を具体的に実装したのが連続円盤と N 体集積の接続であり、外側の集積・生存条件を与える（Canup & Salmon, 2018）。また外側の形成機構と生存の難しさは、内側起源の衛星移動や長期進化を含むモデルで議論されている（Rosenblatt & Charnoz, 2012; Rosenblatt et al., 2016）。

一方、巨大衝突直後の高温環境では、放射圧や昇華により短期的に物質が失われ得ることが示唆されている（Hyodo et al., 2018）。しかし、この短期損失の積算は、拡散・集積モデルの主計算には取り込まれていない。したがって本研究は、**ロッシュ限界内で失われる成分**を、微粒子の生成と除去を結合したモデルで評価し、内側貯蔵庫の入力を更新することに焦点を置く。ここで「損失」は、放射圧除去や昇華によりロッシュ限界内の固体質量が短時間で内側領域から除かれることとして扱い、必ずしも火星重力圏の外まで出ることを意味しない。

以下の図は、先行研究の入力と出力、そして内側→外側の結び目と未解決点を整理したものであり、各項目の代表文献をノード内に著者年で示す。

```mermaid
flowchart TB
    subgraph SPH["衝突直後の初期条件 (SPH)"]
        A[巨大衝突]
        B[SPH出力: 質量分布/温度/相/粒径<br/>Hyodo et al. 2017<br/>Hyodo et al. 2018<br/>Citron et al. 2015]
    end

    subgraph INNER["ロッシュ限界内 (内側貯蔵庫)"]
        C[内側質量集中が既知<br/>Citron et al. 2015<br/>Rosenblatt et al. 2016]
        D[供給ゲート: 粘性拡散で外側へ供給<br/>Salmon et al. 2010<br/>Crida and Charnoz 2012]
    end

    subgraph OUTER["ロッシュ限界外 (外側集積)"]
        E[内側-外側の結び目<br/>連続円盤とN体の接続<br/>Canup and Salmon 2018]
        F[外側集積と生存条件の出力<br/>Rosenblatt and Charnoz 2012<br/>Rosenblatt et al. 2016]
        G[Phobos Deimos]
    end

    subgraph LOSS["短期損失（未解決）"]
        H[放射圧/昇華/凝縮粒子除去<br/>Hyodo et al. 2018]
        I[本研究: 内側短期損失を定量化]
    end
