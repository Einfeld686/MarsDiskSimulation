...
| `_log_stage` | L100 | ヘルパー | Emit coarse progress markers for debugging long ru... |
| `_resolve_los_factor` | L106 | ヘルパー | Return the multiplicative factor f_los scaling τ_v... |
| `_surface_energy_floor` | L122 | ヘルパー | Return surface-energy-limited minimum size (Krijt ... |
| `_auto_chi_blow` | L146 | ヘルパー | Return an automatic chi_blow scaling based on β an... |
| `_fast_blowout_correction_factor` | L161 | ヘルパー | Return the effective loss fraction ``f_fast = 1 - ... |
| `load_config` | L184 | 関数 | Load a YAML configuration file into a :class:`Conf... |
| `_gather_git_info` | L209 | ヘルパー | Return basic git metadata for provenance recording... |
| `MassBudgetViolationError` | L236 | クラス | Raised when the mass budget tolerance is exceeded. |
| `run_zero_d` | L242 | メイン | Execute the full-feature zero-dimensional simulati... |
| `main` | L4422 | 関数 | Command line entry point. |

## 3. 主要セクション（目安）

> 以下の行範囲はコード変更により変動します。`inventory.json` を基に自動更新されます。

- **`run_zero_d()`**: L242–? (メイン実行ドライバ)
- **`main()`**: L4422–? (CLI エントリポイント)
## 4. 探索ガイド

| 調べたいこと | 参照シンボル | 備考 |
|-------------|-------------|------|
| 設定ロード | [`load_config`](L184) | YAML→Config変換 |
| 時間グリッド | `_resolve_time_grid` | dt, n_steps決定 (未検出) |
| シード解決 | `_resolve_seed` | RNG初期化 (未検出) |
| 高速ブローアウト補正 | [`_fast_blowout_correction_factor`](L161) | dt/t_blow補正 |
