...
| `SmolSinkWorkspace` | L105 | クラス | No description available. |
| `_get_smol_sink_workspace` | L114 | ヘルパー | No description available. |
| `_get_max_steps` | L134 | ヘルパー | Return MAX_STEPS, honoring overrides applied to th... |
| `_log_stage` | L148 | ヘルパー | Emit coarse progress markers for debugging long ru... |
| `_model_fields_set` | L154 | ヘルパー | Return explicitly-set fields for a Pydantic model ... |
| `_resolve_los_factor` | L165 | ヘルパー | Return the multiplicative factor f_los scaling τ_v... |
| `_surface_energy_floor` | L181 | ヘルパー | Return surface-energy-limited minimum size (Krijt ... |
| `_auto_chi_blow` | L220 | ヘルパー | Return an automatic chi_blow scaling based on β an... |
| `_fast_blowout_correction_factor` | L235 | ヘルパー | Return the effective loss fraction ``f_fast = 1 - ... |
| `load_config` | L258 | 関数 | Load a YAML configuration file into a :class:`Conf... |
...
| 調べたいこと | 参照シンボル | 備考 |
|-------------|-------------|------|
| 設定ロード | [`load_config`](L258) | YAML→Config変換 |
| 時間グリッド | `_resolve_time_grid` | dt, n_steps決定 (未検出) |
| シード解決 | `_resolve_seed` | RNG初期化 (未検出) |
| 高速ブローアウト補正 | [`_fast_blowout_correction_factor`](L235) | dt/t_blow補正 |
