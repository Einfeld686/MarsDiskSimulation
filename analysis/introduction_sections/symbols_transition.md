...
本ファイルは、現時点の導入（遷移期）文書に現れる記号を、TeX 化に先立って一時的に整理したものである。定義が先行研究依存で確定できない項目は TODO(REF:transition_symbols_pending_refs_v1) として残している。
...
\begin{table}[t]
  ...
  \begin{tabular}{p{0.20\linewidth}p{0.46\linewidth}p{0.12\linewidth}p{0.18\linewidth}}
    ...
    $\mathrm{kg}$ &
    TODO(REF:canup2018_min_definition_v1) \\
    $M_{\rm in}^{\rm SPH}$ &
    SPH 終端時刻におけるロッシュ限界内側の円盤質量（接続前の推定値） &
    $\mathrm{kg}$ &
    SPH 出力から集計 TODO(REF:sph_mass_aggregation_method_v1) \\
    $\Delta M_{\rm in}$ &
    遷移期における内側円盤の不可逆損失（表層散逸・不可逆落下等の総和） &
    $\mathrm{kg}$ &
    本研究で評価対象 TODO(REF:delta_min_infall_policy_v1) \\
    $M_{\rm in,0}$ &
    ...
    TODO(REF:transition_start_time_definition_v1) \\
    $r_d$ &
    内側円盤の外縁（半径） &
    $\mathrm{m}$（または $R_{\rm Mars}$） &
    TODO(REF:canup2018_rd_definition_v1) \\
    $a_{\rm eq,max}$ &
    円盤が赤道面へ緩和した後の「最大半長軸」等を表す候補記号 &
    未定 &
    TODO(REF:aeqmax_rd_relation_v1) \\
    $J_2$ &
...
- 先行研究（特に Canup \& Salmon (2018)）の該当箇所を確認し、(i) 同一概念ならどちらかに統一する、(ii) 別概念なら本文で初出定義を与える、のいずれかを行う必要がある。TODO(REF:aeqmax_rd_relation_v1)
