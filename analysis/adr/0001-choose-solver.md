...
- 既存の `analysis/equations.md (E.010)` / `(E.011)` では IMEX-BDF(1) の安定条件（lossを陰的、gain/sinkを陽的）と質量誤差測定を共有し、`marsdisk/physics/smol.py#psd_state_to_number_density [L44–L107]` が単ステップの adaptivity (`safety=0.1`) と質量許容値 (`mass_tol=5e-3`) を保証している。

## 決定
IMEX-BDF(1) を `marsdisk/physics/smol.py#psd_state_to_number_density [L44–L107]` で正式採用し、loss（$C_{ij}$ に由来する $\Lambda_i$）は陰的解法、gain と外部 sink `S_k` は陽的に扱う。時間刻みは
...
Surface/PSD モジュールは `run_zero_d` から得る `prod_subblow_mass_rate` と `S_k` を IMEX solver に渡し、更新後の `N_k` を `analysis/equations.md (E.007)` の表層 ODE に接続して `M_out_dot` を再評価する。[marsdisk/run_zero_d.py#run_zero_d [L242–L4419]]
