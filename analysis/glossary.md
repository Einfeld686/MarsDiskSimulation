...
| G.A08 | deep_mixing transport | both | light | code:supply.py | draft | A | 表層再供給を深部リザーバー経由で表層へ混合する輸送モード。`supply.transport.mode="deep_mixing"` で有効化し、`t_mix_orbits` で混合時間を指定する。τ>1 での供給過多を避けつつ質量を保存するための経路。 |  | code-internal |
...
| G.A10 | surface-layer replenishment | both | light | code:supply.py | draft | A | 「外部供給」ではなく「表層再供給」と呼ぶのは、外側からの流入を意味せず、深部↔表層の入れ替わりを粗く表現するパラメータであることを明示するため。供給率は上下限評価のノブとして扱い、物理的な流入源の確定を意図しない。 |  | code-internal |
| G.A11 | epsilon_mix | both | light | code:supply.py | draft | A | バリスティック混合効率を表す無次元係数（0–1）。表層再供給に掛かる実効スケールで、MU_LIST として研究スイープで掃引される。`supply.mixing.epsilon_mix` で設定。 | E.027 | WyattClarkeBooth2011,EstradaDurisen2015 |
...
| 変数名 | 物理的意味 | 式 | 出典 |
|--------|-----------|-----|------|
| `t_blow` | ブローアウト時間 | χ/Ω | [@StrubbeChiang2006_ApJ648_652] |
| `t_coll` | 衝突時間 | 1/(Ω τ) | [@Wyatt2008] |
| `t_sink` | シンク総合時間 | 調和平均 | - |
| `t_orb` | 公転周期 | 2π/Ω | - |
