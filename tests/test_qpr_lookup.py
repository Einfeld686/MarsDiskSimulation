...
def test_qpr_lookup_interpolates_from_table(tmp_path):
    path = _make_mock_table(tmp_path)
    ...
    try:
        ...
        rho = 1500.0
        beta_val = radiation.beta(mid_s, rho, mid_T, table=lookup)
...
def test_qpr_lookup_fallback_and_validation():
    original_table = tables._QPR_TABLE
    original_lookup = radiation._QPR_LOOKUP
    try:
