...
def qpr_interp_array_numba(
    ...
    """Bilinear interpolation for the Planck-mean Q_pr table over an array of sizes."""
    n_s = s_vals.shape[0]
    n_T = T_vals.shape[0]
    out = np.empty_like(s_arr, dtype=np.float64)
    ...
    for idx in range(s_arr.shape[0]):
        s = s_arr[idx]
        i = np.searchsorted(s_vals, s) - 1
        if i < 0:
            i = 0
