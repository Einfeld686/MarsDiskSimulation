#!/usr/bin/env bash
# Run sweep-parallel smoke checks for structural validation.
...
usage() {
  cat <<'USAGE'
Usage:
  run_sweep_parallel_smoke_check.sh [--no-venv] [--no-install] [--] [args...]
...
cd "${REPO_ROOT}"
if ((${#PASS_ARGS[@]})); then
  exec "${PYTHON_EXE}" scripts/tests/sweep_parallel_smoke_check.py "${PASS_ARGS[@]}"
fi
exec "${PYTHON_EXE}" scripts/tests/sweep_parallel_smoke_check.py
