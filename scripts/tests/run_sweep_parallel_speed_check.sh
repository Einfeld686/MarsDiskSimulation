#!/usr/bin/env bash
# Run a short sweep-parallel on/off speed check.
...
usage() {
  cat <<'USAGE'
Usage:
  run_sweep_parallel_speed_check.sh [--no-venv] [--no-install] [--] [args...]
...
cd "${REPO_ROOT}"
if ((${#PASS_ARGS[@]})); then
  exec "${PYTHON_EXE}" scripts/tests/sweep_parallel_speed_check.py "${PASS_ARGS[@]}"
fi
exec "${PYTHON_EXE}" scripts/tests/sweep_parallel_speed_check.py
