...
if not defined OUT_ROOT if "%AUTO_OUT_ROOT%"=="1" (
  ...
  set "FREE_GB="
  for /f "usebackq delims=" %%F in (`call "%PYTHON_EXEC_CMD%" -c "import os,shutil; p=os.environ.get('CHECK_PATH',''); print(shutil.disk_usage(p).free//(1024**3) if p and os.path.exists(p) else '')"`) do set "FREE_GB=%%F"
...
rem Per-process thread cap (sweep-parallel uses 3, cell-parallel uses 3).
if "%SWEEP_PARALLEL%"=="1" (
...
if defined CI (
  call "%PYTHON_EXEC_CMD%" "%REPO_ROOT%\\scripts\\runsets\\windows\\preflight_checks.py" --repo-root "%REPO_ROOT%" --config "%CONFIG_PATH%" --overrides "%OVERRIDES_PATH%" --out-root "%OUT_ROOT%" --python-exe "!PYTHON_EXE!" --require-git --cmd "%REPO_ROOT%\\scripts\\research\\run_temp_supply_sweep_eps1p5.cmd" --cmd-root "%REPO_ROOT%\\scripts\\runsets\\windows" --cmd-root "%REPO_ROOT%\\scripts\\research" --cmd-exclude "%REPO_ROOT%\\scripts\\runsets\\windows\\legacy" --cmd-allowlist "%REPO_ROOT%\\scripts\\runsets\\windows\\preflight_allowlist.txt" --allow-non-ascii --profile ci --format json %PREFLIGHT_STRICT_FLAG%
) else (
...
call "%PYTHON_EXEC_CMD%" "%REPO_ROOT%\\scripts\\runsets\\common\\read_overrides_cmd.py" --file "%OVERRIDES_PATH%" --out "%ARCHIVE_SET%"
...
if not exist "%REPO_ROOT%\scripts\research\run_temp_supply_sweep_eps1p5.cmd" (



  echo.[error] run_temp_supply_sweep_eps1p5.cmd not found: "%REPO_ROOT%\scripts\research\run_temp_supply_sweep_eps1p5.cmd"
...
if "%DRY_RUN%"=="1" (
  ...
  %LOG_INFO% dry-run: calling run_temp_supply_sweep_eps1p5.cmd



  call scripts\research\run_temp_supply_sweep_eps1p5.cmd --dry-run
...
if "%SWEEP_PARALLEL%"=="1" if not "%SIZE_PROBE_ENABLE%"=="0" if "%PARALLEL_JOBS_DEFAULT%"=="1" (
  ...
  for /f "usebackq delims=" %%J in (`call "%PYTHON_EXEC_CMD%" scripts\\tests\\measure_case_output_size.py --batch-root "!SIZE_PROBE_BATCH_ROOT!" --sweep-tag size_probe --t %SIZE_PROBE_T% --eps %SIZE_PROBE_EPS% --tau %SIZE_PROBE_TAU% --batch-seed %SIZE_PROBE_SEED% --hooks "%SIZE_PROBE_HOOKS%" --overrides "%OVERRIDES_PATH%" --reserve-gb %SIZE_PROBE_RESERVE_GB% --safety-fraction %SIZE_PROBE_FRACTION% --temp-root "%TEMP_ROOT%" --skip-pip --print-recommended-jobs --quiet`) do set "SIZE_PROBE_JOBS=%%J"
...
rem ::REF:CHILD_RUN
%LOG_INFO% launching run_temp_supply_sweep_eps1p5.cmd
if "%DEBUG%"=="1" call :debug_log "run_temp_supply: start cmd=scripts\research\run_temp_supply_sweep_eps1p5.cmd"
...
call scripts\research\run_temp_supply_sweep_eps1p5.cmd
...
%LOG_INFO% run_temp_supply_sweep_eps1p5.cmd finished (rc=%RUN_RC%)
